...
COMPILEALL_VERIFY := PYTHONPYCACHEPREFIX=/tmp/mimisee-pycache $(PY) -m compileall -q app
ifeq ($(OS),Windows_NT)
COMPILEALL_VERIFY := $(PY) -m compileall -q app
endif

COMPOSE_PROJECT_NAME ?= mimisee
...
PLUGIN_RELEASE_GATE_OUT ?= /tmp/mimisee_plugin_release_gate.json
...
MIMISEE_TENANT_ID ?= 00000000-0000-0000-0000-000000000000
MIMISEE_ACCOUNT_ID ?= demo
MIMISEE_USER_ID ?= demo
MIMISEE_API_TOKEN ?= $(AUTH_TOKEN)
MIMISEE_API_TIMEOUT ?= 60
...
help:
	@echo "MimiSee dev commands (run from repo root):"
	...
	@echo "  make down      - stop/remove all MimiSee Compose containers and networks; keep data/images"
	@echo "  make docker-reset - also delete MimiSee named volumes and all persisted data"
	@echo "  make docker-purge - also delete images used by MimiSee services"
	...
	@echo "  make restart   - docker compose restart mimisee-api"
	...
	@echo "  make helm-template - helm template smoke (deploy/helm/mimisee)"
	@echo "  make helm-lint  - helm lint (deploy/helm/mimisee)"
...
models:
	@$(PY) scripts/bootstrap_mimisee_models.py
...
up-docling: init
	DOCLING_ENABLED_DOCKER=true DOCLING_API_URL_DOCKER=http://mimisee-docling:5001 $(COMPOSE_PARSERS) --profile docling up -d --build

up-docling-gpu: init
	DOCLING_ENABLED_DOCKER=true DOCLING_API_URL_DOCKER=http://mimisee-docling:5001 $(COMPOSE_PARSERS) --profile docling-gpu up -d --build
...
mixed-rag-quality:
	...
	@test -n "$(MIXED_RAG_RUNS)" || (echo "MIXED_RAG_RUNS is required, e.g. MIXED_RAG_RUNS='mimisee=/tmp/mimisee.json dify=/tmp/dify.json'" >&2; exit 2)
...
compileall-docker:
	$(COMPOSE) exec -T -e PYTHONPYCACHEPREFIX=/tmp/mimisee-pycache -w /app mimisee-api python -m compileall -q app
...
# - Chroma: MimiSee only reaches the embedded local LangChain Chroma path
...
# - Ragas: MimiSee does not call the affected multimodal URL evaluator.
...
helm-template:
	helm template mimisee deploy/helm/mimisee -n mimisee >/dev/null
