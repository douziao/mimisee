...
MimiSee evaluates evidence retrieval separately from answer generation:
...
Dify can consume MimiSee through two paths:

- **External Knowledge API**: `POST /api/v1/integrations/dify/retrieval`.
- **Workflow HTTP node**: Dify sends the query, dataset scope, and filters; MimiSee returns evidence and trace data.

Dify owns orchestration and generation, while MimiSee keeps governance, retrieval, reranking, permission filtering, and evidence. Use [OpenAPI](https://douziao.github.io/mimisee/) for the current request schema.
...
The last two commands are irreversible. Complete a backup and restore drill first. MimiSee uses the isolated Compose project name `mimisee`; see [Deployment](../ops/deployment) for PowerShell, Dify coexistence, and legacy recovery. Do not substitute a global `docker system prune` for project-scoped cleanup.
...
The complete offline repository manual is [MimiSee Full Operation Guide](https://github.com/douziao/mimisee/blob/main/docs/user_guide.md).
