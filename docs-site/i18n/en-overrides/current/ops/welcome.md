...
This handbook is for **operations engineers, SREs, and platform administrators**, covering MimiSee's deployment methods, health checks, observability, and routine operations.
...
With `PROMETHEUS_ENABLED=true`, MimiSee exposes a Prometheus-compatible `/metrics` endpoint:
...
Monitor Arq, PostgreSQL, Milvus, Redis, and MinIO through application logs, `make worker-check`, and dependency-specific exporters; MimiSee does not expose queue-specific metrics directly.
...
| `deploy/helm/mimisee/` | Helm Chart templates |
