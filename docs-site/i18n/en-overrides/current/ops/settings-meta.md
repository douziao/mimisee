...
MimiSee reads process environment variables and the repository-root `.env` through Pydantic Settings. The authoritative definitions are [`.env.example`](https://github.com/douziao/mimisee/blob/main/.env.example) and `app/core/config.py`.
