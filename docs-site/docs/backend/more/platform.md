...
MimiSee 提供完整的多租户平台能力，包括租户管理、用户角色权限（RBAC）、SCIM 自动同步和用量审计。
...
MimiSee 采用单库多租户架构，所有数据表通过 `tenant_id` 列实现逻辑隔离：
...
MimiSee 实现 SCIM 2.0 协议端点，支持与企业 IdP（如 Azure AD、Okta）自动同步用户和组：
