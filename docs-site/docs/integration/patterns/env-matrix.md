...
MimiSee 的配置主要分成三类：本地开发、Docker 一键启动、生产部署。权威变量定义见 [设置 / Meta](../../ops/settings-meta.md)。
...
| Docker 一键启动 | `NEXT_PUBLIC_API_URL_DOCKER=/`、`API_INTERNAL_URL_DOCKER=http://mimisee-api:8000` | `make init` 后填写 `LLM_API_KEY` | 适合 `make up-web`；本地基础设施可保留默认值 |
