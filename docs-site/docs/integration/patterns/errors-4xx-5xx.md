...
MimiSee API 遵循标准 HTTP 状态码语义，错误响应包含结构化的错误信息，便于客户端分类处理。
...
:::info
具体字段名以 [Redoc](https://douziao.github.io/mimisee/) 中 `ErrorResponse` schema 为准。部分接口可能包含额外的 `field` 或 `errors` 数组字段。
:::
