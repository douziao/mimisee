...
MimiSee 支持多种认证方式，生产环境推荐 JWT Bearer Token，开发环境可使用 Header 调试模式。
...
返回 `access_token` 与 `refresh_token`（字段名以 [Redoc](https://douziao.github.io/mimisee/) 为准）。
...
:::info
API Key 的创建与管理通过管理后台操作，具体接口见 [Redoc](https://douziao.github.io/mimisee/) 中 **auth** 分组。
:::
