...
MimiSee 通过 `multipart/form-data` 接口上传文档，支持 PDF、Office、纯文本等格式。大文件场景可使用预签名 URL 直传对象存储。
...
:::warning 字段名
`file` 等表单字段名必须与 [Redoc](https://douziao.github.io/mimisee/) 中 **documents** 分组的定义严格一致。使用错误的字段名会返回 400 或 415。
:::
...
sequenceDiagram
    participant Client
    participant API as MimiSee API
