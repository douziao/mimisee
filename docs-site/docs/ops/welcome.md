...
本手册面向 **运维工程师、SRE 与平台管理员**，覆盖 MimiSee 的部署方式、健康检查、可观测性与日常运维操作。
...
设置 `PROMETHEUS_ENABLED=true` 后，MimiSee 暴露 Prometheus 兼容的 `/metrics` 端点：
...
Arq 队列、PostgreSQL、Milvus、Redis 与 MinIO 的运行状态应结合应用日志、`make worker-check` 和各依赖自身的 exporter 监控；MimiSee 不会直接暴露队列层的专用指标。
...
| `deploy/helm/mimisee/` | Helm Chart 模板 |
