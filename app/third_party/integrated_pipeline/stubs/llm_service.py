...
tenant-scoped LLM services. MimiSee does not ship the full Integrated pipeline backend,
    ...
    def __init__(self, tenant_id: str, llm_type: str, **kwargs: Any):
        ...
        if not bool(getattr(settings, "VISION_LLM_ENABLED", False)):
            raise NotImplementedError(
                "Vision enrichment is disabled. Set MIMISEE_VISION_LLM_ENABLED=true to enable image-to-text parsing."
            )
        ...
        if not api_key:
            raise NotImplementedError(
                "Vision enrichment requires an API key. Set MIMISEE_VISION_LLM_API_KEY (or LLM_API_KEY)."
            )
        if not api_base:
            raise NotImplementedError(
                "Vision enrichment requires an API base. Set MIMISEE_VISION_LLM_API_BASE (or LLM_API_BASE)."
            )
        if not model:
            raise NotImplementedError(
                "Vision enrichment requires a model. Set MIMISEE_VISION_LLM_MODEL (or pass llm_name)."
            )
