...
FUSION_WEIGHT_OBSERVABILITY_SCHEMA = "mimisee.fusion_weight_observability.v1"
TENANT_FUSION_WEIGHTS_SCHEMA = "mimisee.tenant_fusion_weights.v1"
...
def _extract_trace_payload(row: Mapping[str, Any]) -> tuple[dict[str, Any] | None, str]:
    ...
    if str(row.get("schema") or "").strip() == "mimisee.training_export_row.v1":
...
def _is_ltr_training_ready_row(*, row: Mapping[str, Any], trace: Mapping[str, Any]) -> bool:
    return (
        str(row.get("schema") or "").strip() == "mimisee.training_export_row.v1"
