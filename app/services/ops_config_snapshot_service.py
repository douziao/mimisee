...
def _get_build_time() -> str | None:
    value = (os.getenv("MIMISEE_BUILD_TIME") or os.getenv("BUILD_TIME") or "").strip()
    return value or None
...
def build_ops_config_snapshot() -> OpsConfigSnapshot:
    ...
    return OpsConfigSnapshot(schema="mimisee.ops_config_snapshot.v1", fingerprint=fp, config=cfg)
