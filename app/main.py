...
logger = logging.getLogger("mimisee")
_OPENAPI_EXPORT_MODE = str(os.getenv("MIMISEE_OPENAPI_EXPORT", "") or "").strip().lower() in {
    "1",
    "true",
    "yes",
    "y",
...
async def lifespan(app: FastAPI):
    ...
    logger.info("Starting MimiSee backend...")
    ...
    # Shutdown cleanup.
    logger.info("Shutting down MimiSee backend...")
...
# Create FastAPI app
app = FastAPI(
    title="MimiSee - Knowledge Base RAG System",
...
async def root():
    """Root path."""
    return {"message": "Welcome to MimiSee API", "version": __version__, "docs": _DOCS_PATH}
