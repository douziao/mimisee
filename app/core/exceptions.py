"""
MimiSee API unified exception handling module
"""
...
class MimiSeeError(Exception):
    """Base exception for MimiSee application."""
...
MimiSeeException = MimiSeeError


class ValidationError(MimiSeeError):
    def __init__(
        self,
        message: str,
        field: str | None = None,
...
class RateLimitError(MimiSeeError):
...
def mimisee_exception_handler(request: Request, exc: MimiSeeError) -> JSONResponse:
    request_id = get_request_id(request)
    logger.warning(
        "MimiSee exception: %s - %s (request_id=%s)",
        exc.error_code,
        exc.message,
        request_id,
    )
...
def register_exception_handlers(app: FastAPI) -> None:
    app.add_exception_handler(MimiSeeError, mimisee_exception_handler)
