...
_KG_EXTRACT_EVENT_SCHEMA_ZH = render_formal_json_prompt(
    ...
    task_rules=[
        (
            ...
            "（Apache-2.0），仅用于抽取结构，不改变 MimiSee 召回排序。"
        ),
...
_JUDGE_CITATION_CORRECTNESS_ZH = render_formal_json_prompt(
    ...
    objective="逐条核对答案中的引用是否真实存在、是否真正支持被引用的结论(MimiSee rag-evaluation P0 护城河 metric)。",
    task_rules=[
...
_BUILTIN_PROMPT_TEMPLATES: tuple[BuiltinPromptTemplate, ...] = (
    ...
    BuiltinPromptTemplate(
        ...
        name="引用正确性评测（护城河指标）",
        description="逐条核对答案中引用的真实性与支持度。灵感来源：MimiSee rag-evaluation P0 护城河 metric。",
