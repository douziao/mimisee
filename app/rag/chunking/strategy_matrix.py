...
FIXTURES: dict[str, StrategyFixture] = {
    "generic_text": _fixture(
        "generic.txt",
        """
        MimiSee runs retrieval, governance, and chunking for multi-format corpora.
    ...
    "docker_compose": _fixture(
        ...
        services:
          api:
            image: mimisee/api:latest
            ports: ["8000:8000"]
          web:
            image: mimisee/web:latest
            ports: ["3000:3000"]
        """,
        file_type="yaml",
    ),
    ...
    for strategy in sorted(
        ...
        try:
            if strategy in chunker_factory.INTEGRATED_PIPELINE_STRATEGIES:
                with tempfile.TemporaryDirectory(prefix="mimisee-strategy-matrix-") as tmpdir:
