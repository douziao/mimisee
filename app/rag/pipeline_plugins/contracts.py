...
def parse_metadata_schema(schema: dict[str, Any] | None) -> list[MetadataField]:
    if not schema:
        return []
    if schema.get("schema") != "mimisee.metadata_schema.v1":
        raise PipelinePluginContractError("metadata_schema.schema must be mimisee.metadata_schema.v1")
...
def validate_golden_rules_metadata_fields(
    *,
    ...
    if not isinstance(golden_rules, dict) or golden_rules.get("schema") != "mimisee.golden_rules.v1":
        return
...
def validate_retrieval_text_schema_metadata_fields(
    *,
    ...
    if not isinstance(retrieval_text_schema, dict) or (
        retrieval_text_schema.get("schema") != "mimisee.retrieval_text_schema.v1"
    ):
        return
...
def validate_retrieval_policy_metadata_fields(
    *,
    ...
    if not isinstance(retrieval_policy, dict) or retrieval_policy.get("schema") != "mimisee.retrieval_policy.v1":
        return
...
def _record_identity_payload(meta: dict[str, Any], schema: dict[str, Any] | None) -> dict[str, Any] | None:
    ...
    return {
        "schema": "mimisee.record_identity.v1",
        "key": key[:1000],
        "fields": values,
    }
...
def _retrieval_stage_spec(schema: dict[str, Any] | None, stage: str) -> dict[str, Any]:
    if not schema:
        return {}
    if schema.get("schema") != "mimisee.retrieval_text_schema.v1":
        raise PipelinePluginContractError("retrieval_text_schema.schema must be mimisee.retrieval_text_schema.v1")
