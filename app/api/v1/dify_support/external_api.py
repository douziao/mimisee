...
from app.services.external_conversation_ingest import _mimisee_citations_for_storage
...
class DifyRetrievalSetting(BaseModel):
    ...
    # MimiSee extension: "fast" keeps online chat latency low by disabling
    ...
    # MimiSee-compatible extension for direct gates and internal probes.
...
class DifyExternalKnowledgeRequest(BaseModel):
    ...
    # MimiSee extension: lets Dify HTTP/workflow calls attach retrieval traces
...
def _dify_trace_source_conversation_id(request: Request, body: DifyExternalKnowledgeRequest) -> str | None:
    ...
    return _first_nonempty_str(
        ...
        request.headers.get("x-mimisee-source-conversation-id"),
...
def _dify_trace_source_message_id(request: Request, body: DifyExternalKnowledgeRequest) -> str | None:
    return _first_nonempty_str(
        ...
        request.headers.get("x-mimisee-source-message-id"),
...
def _dify_trace_source_run_id(request: Request, body: DifyExternalKnowledgeRequest) -> str | None:
    return _first_nonempty_str(
        ...
        request.headers.get("x-mimisee-source-run-id"),
...
def _dify_turn_citations_for_storage(citations: list[dict[str, Any]]) -> list[dict[str, Any]]:
    return _mimisee_citations_for_storage([item for item in citations or [] if isinstance(item, dict)])
...
def _dify_trace_conversation_id(
    ...
    *,
    ...
    for header_name in ("x-mimisee-conversation-id", "x-conversation-id"):
...
def _dify_trace_request_id(request: Request, body: DifyExternalKnowledgeRequest) -> str | None:
    return _first_nonempty_str(
        body.request_id,
        request.headers.get("x-mimisee-request-id"),
