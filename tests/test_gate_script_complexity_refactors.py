...
@pytest.mark.parametrize(
    ...
    [
        (
            answer_quality_gate,
            "mimisee.answer_quality_thresholds.v1",
            "mimisee.answer_quality_gate_report.v1",
        ...
        (
            parsing_retrieval_proof_gate,
            "mimisee.parsing_retrieval_proof_thresholds.v1",
            "mimisee.parsing_retrieval_proof_gate_report.v1",
...
def test_evidence_pack_conversion_prefers_explicit_sources_and_falls_back_to_citations() -> None:
    ...
    assert explicit_bundle == {
        "schema": "mimisee.regression_cases.v1",
        "dataset_id": "dataset-a",
        "items": [
            {
...
def test_export_diagnostics_summarizes_sorted_counts_and_jsonl_inputs(tmp_path: Path) -> None:
    ...
    assert payload["schema"] == "mimisee.export_diagnostics.v1"
...
def test_must_recall_gate_cli_writes_output_and_reports_errors(
    ...
    assert stdout_payload["schema"] == "mimisee.must_recall_provenance_gate.v1"
...
def test_validate_governance_normalizes_lists_and_cli_reports_failures(
    ...
    payload = {
        "schema": "mimisee.parsing_retrieval_proof_governance.v1",
        "mode": " WARN ",
    ...
    assert normalized["schema"] == "mimisee.parsing_retrieval_proof_governance.v1"
...
def test_parse_repair_gate_extracts_nested_tail_and_clamps_cli_policy(
    ...
    assert report == {
        "schema": "mimisee.parse_repair_gate_report.v1",
