...
def test_semantic_cache_retention_defaults_stay_safe_and_bounded() -> None:
    values = _yaml("deploy/helm/mimisee/values.yaml")
...
def test_semantic_cache_retention_prod_example_closes_the_loop() -> None:
    values = _yaml("deploy/helm/mimisee/examples/values-prod.yaml")
...
def test_semantic_cache_retention_template_and_schema_expose_expected_knobs() -> None:
    template = _read("deploy/helm/mimisee/templates/cronjob-semantic-cache-retention.yaml")
    ...
    assert 'include "mimisee.secretName"' in template
    assert 'include "mimisee.serviceAccountNameBlock"' in template

    schema = json.loads(_read("deploy/helm/mimisee/values.schema.json"))
...
def test_helm_template_renders_semantic_cache_retention_cronjob_from_prod_values() -> None:
    ...
    cronjob = next(
        doc
        for doc in docs
        if doc.get("kind") == "CronJob" and doc.get("metadata", {}).get("name") == "mimisee-semantic-cache-retention"
    )
    ...
    assert container["envFrom"] == [{"secretRef": {"name": "mimisee-env"}}]
