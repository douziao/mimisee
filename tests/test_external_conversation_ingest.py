...
from app.services.external_conversation_ingest import (
    ...
    _mimisee_citations_for_storage,
...
def test_external_conversation_ingest_can_ack_async_for_dify(monkeypatch):
    ...
    response = client.post(
        "/api/v1/integrations/conversations/ingest",
        headers={"X-MimiSee-Async-Ingest": "true"},
        json={
            "source": "dify",
...
def test_external_citations_are_stored_only_when_mimisee_schema_compatible():
    valid = {
    ...
    assert _mimisee_citations_for_storage([valid, external_only]) == [valid]
...
def test_external_conversation_ingest_normalizes_dify_http_records_from_metadata():
    ...
    stored = _mimisee_citations_for_storage(candidates)
