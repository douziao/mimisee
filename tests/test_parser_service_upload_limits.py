...
def test_parser_host_ports_bind_to_loopback() -> None:
    ...
    assert compose["services"]["mimisee-docling"]["ports"] == ["127.0.0.1:${DOCLING_PORT:-5001}:5001"]
    assert compose["services"]["mimisee-docling-gpu"]["ports"] == [
        "127.0.0.1:${DOCLING_PORT:-5001}:5001"
    ]
    assert compose["services"]["mimisee-marker"]["ports"] == ["127.0.0.1:2080:2080"]
    assert compose["services"]["mimisee-magicpdf"]["ports"] == ["127.0.0.1:2095:2095"]
    assert compose["services"]["mimisee-qianfanocr"]["ports"] == ["127.0.0.1:2090:2090"]
...
def test_gpu_parsers_use_compose_compatible_device_reservations() -> None:
    ...
    for service_name in gpu_services:
        ...
        if service_name == "mimisee-docling-gpu":
...
def test_docling_is_a_pinned_external_service_not_a_main_runtime_dependency() -> None:
    compose = yaml.safe_load(Path("docker/docker-compose.parsers.yml").read_text(encoding="utf-8"))
    service = compose["services"]["mimisee-docling"]
...
def test_docling_gpu_service_is_pinned_and_conservative_by_default() -> None:
    compose = yaml.safe_load(Path("docker/docker-compose.parsers.yml").read_text(encoding="utf-8"))
    service = compose["services"]["mimisee-docling-gpu"]
