...
def _demo_policy(**overrides: object) -> dict[str, object]:
    policy: dict[str, object] = {
        "schema": "mimisee.retrieval_policy.v1",
...
def test_dify_resolve_knowledge_policy_helpers_use_mapping_plugin_refs(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(
        ...
        "_retrieval_policy_for_plugin_ref",
        lambda ref: {
            "plugin-a": {"schema": "mimisee.retrieval_policy.v1", "filter_fields": ["category"]},
            "plugin-b": {"schema": "mimisee.retrieval_policy.v1", "filter_fields": ["district"]},
        }.get(ref, {}),
        raising=True,
    )
    monkeypatch.setattr(
...
def test_dify_trace_conversation_id_rejects_access_to_other_account_conversation(
    ...
    request = SimpleNamespace(headers={"x-mimisee-conversation-id": str(conversation_id)})
...
def test_dify_metadata_condition_is_converted_to_mimisee_filter() -> None:
...
def test_dify_record_conversion_uses_mimisee_score_from_metadata() -> None:
    from app.api.v1.integrations_dify import _citation_to_dify_record
    ...
    assert record["metadata"]["mimisee_score"] == 0.89
...
def test_dify_trace_context_accepts_body_and_header_extensions() -> None:
    ...
    request = SimpleNamespace(
        headers={
            "x-mimisee-conversation-id": str(uuid.uuid4()),
            "x-mimisee-request-id": "header-request-id",
        },
    ...
    header_request = SimpleNamespace(
        headers={
            "x-mimisee-conversation-id": str(header_conversation_id),
            "x-mimisee-request-id": "header-request-id",
        },
        state=SimpleNamespace(request_id="state-request-id"),
    )
...
def test_dify_trace_context_auto_creates_mimisee_conversation_for_dify_conversation(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    from app.api.v1 import integrations_dify as dify_api
