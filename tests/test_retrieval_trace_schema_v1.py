...
def test_orchestrator_emits_stable_retrieval_trace_schema(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert trace.get("schema") == "mimisee.retrieval_trace_pass.v1"
    ...
    assert proof.get("schema") == "mimisee.must_recall_proof.v1"
    ledger = proof.get("obligation_ledger") or {}
    assert isinstance(ledger, dict)
    assert ledger.get("schema") == "mimisee.recall_obligation_ledger.v1"
    ...
    assert router_layers.get("schema") == "mimisee.router_layers.v1"
