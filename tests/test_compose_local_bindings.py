...
def test_infrastructure_host_ports_bind_to_loopback() -> None:
    ...
    assert services["mimisee-postgres"]["ports"] == ["127.0.0.1:5432:5432"]
    assert services["mimisee-redis"]["ports"] == ["127.0.0.1:6379:6379"]
    assert services["mimisee-minio"]["ports"] == [
        "127.0.0.1:9001:9001",
        "127.0.0.1:9000:9000",
    ]
    assert services["mimisee-milvus"]["ports"] == [
...
def test_retrieval_dev_api_binds_to_loopback() -> None:
    api = _compose("docker/docker-compose.retrieval-dev.yml")["services"]["mimisee-api"]
...
def test_local_chroma_volume_uses_image_owned_directory() -> None:
    ...
    assert retrieval["services"]["mimisee-api"]["volumes"] == [
    ...
    assert "vector_data:/app/vector_chroma" in lite["services"]["mimisee-api"]["volumes"]
    assert "vector_data:/app/vector_chroma" in lite["services"]["mimisee-worker"]["volumes"]
...
def test_web_compose_uses_docker_scoped_api_env_vars_with_same_origin_default() -> None:
    ...
    assert web["build"]["args"]["NEXT_PUBLIC_ADMIN_CONTACT_URL"] == (
        "${NEXT_PUBLIC_ADMIN_CONTACT_URL:-https://github.com/douziao/mimisee/issues}"
    )
    assert web["build"]["args"]["API_INTERNAL_URL"] == "${API_INTERNAL_URL_DOCKER:-http://mimisee-api:8000}"
    ...
    assert web["environment"]["API_INTERNAL_URL"] == "${API_INTERNAL_URL_DOCKER:-http://mimisee-api:8000}"
...
def test_worker_compose_healthcheck_uses_lightweight_arq_check() -> None:
    for path in ("docker/docker-compose.yml", "docker/docker-compose.lite.yml"):
        worker = _compose(path)["services"]["mimisee-worker"]
