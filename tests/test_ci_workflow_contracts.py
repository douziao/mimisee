...
def test_self_hosted_ci_bootstrap_script_contract() -> None:
    ...
    assert 'CACHE_ROOT="${RUNNER_TOOL_CACHE:-$RUNNER_TEMP}/mimisee"' in script
...
def test_main_ci_runs_database_migrations_and_integrations() -> None:
    ...
    assert 'MIMISEE_INTEGRATION_TESTS: "1"' in workflow
...
def test_python_audit_chroma_exception_stays_bound_to_the_embedded_local_backend() -> None:
    ...
    helm_validation = _read("deploy/helm/mimisee/templates/validate-runtime.yaml")
    ...
    assert "Distributed MimiSee deployments cannot use VECTOR_BACKEND=faiss or VECTOR_BACKEND=chroma." in (
        helm_validation
    )
...
def test_main_ci_runs_all_browser_smoke_specs_and_critical_coverage() -> None:
    ...
    assert "runs-on: [self-hosted, Linux, X64, mimisee]" in web_job
...
def test_main_ci_host_browser_smoke_stays_on_the_live_stack_spec_in_pr_jobs_too() -> None:
    ...
    assert 'API_INTERNAL_URL="http://127.0.0.1:${MIMISEE_RETRIEVAL_API_PORT}"' in browser_smoke
    assert 'NEXT_PUBLIC_API_URL="http://127.0.0.1:${MIMISEE_RETRIEVAL_API_PORT}"' not in browser_smoke
...
def test_retrieval_regression_gate_is_offline_and_reuses_bounded_proof_artifacts() -> None:
    ...
    assert "EMBEDDING_MODEL: mimisee-deterministic-test-v1" in backend_step
...
def test_main_ci_routes_public_prs_to_hosted_smoke_checks() -> None:
    ...
    assert "runner.name == 'mimisee-main-01'" not in workflow
    ...
    assert (
        ...
        "lint deploy/helm/mimisee" in workflow
    )
    assert "template mimisee deploy/helm/mimisee >/dev/null" in workflow
...
def test_host_setup_uses_project_venv_and_complete_cpu_dependencies() -> None:
    ...
    assert "MIMISEE_SMOKE_IDENTIFIER=" in env_example
    assert "MIMISEE_SMOKE_PASSWORD=" in env_example
...
def test_pull_request_lint_and_security_jobs_use_hosted_runners() -> None:
    runner_policy = "runs-on: ${{ github.event_name == 'pull_request' && 'ubuntu-latest' || 'mimisee' }}"
    expected_counts = {
...
def test_docker_ci_supports_cold_web_builds() -> None:
    ...
    assert "/opt/venv/bin/python scripts/bootstrap_mimisee_models.py" in backend_dockerfile
    ...
    assert "EMBEDDING_MODEL_DOCKER: mimisee-deterministic-test-v1" in docker_job
    assert "Diagnose README docker quickstart failure" in docker_job
    assert '"${compose[@]}" logs --tail=300 mimisee-api mimisee-worker' in docker_job
    ...
    assert docker_job.count('api_port=$("${compose[@]}" port mimisee-api 8000') >= 2
    ...
    assert "--web-base-url http://mimisee-web-smoke-${GITHUB_RUN_ID}-${GITHUB_RUN_ATTEMPT}:3000" in docker_job
    assert "--secondary-base-url http://$api2_name:8000" in docker_job
    assert "artifacts/core-e2e.web-dual-api.json" in docker_job
    assert '--name "$api2_name" --no-deps mimisee-api' in docker_job
    ...
    assert "image: ${MIMISEE_BACKEND_IMAGE:-mimisee-backend}" in retrieval_compose
    ...
    assert "EMBEDDING_MODEL_RETRIEVAL_DEV:-mimisee-deterministic-test-v1" in retrieval_compose
...
def test_built_web_jwt_browser_smoke_is_explicitly_wired() -> None:
    ...
    assert "MIMISEE_SMOKE_IDENTIFIER and MIMISEE_SMOKE_PASSWORD must be set together" in script
    ...
    assert "MIMISEE_SMOKE_IDENTIFIER=%s" in script
    assert "MIMISEE_SMOKE_PASSWORD=%s" in script
    ...
    assert "-e MIMISEE_SMOKE_IDENTIFIER" in dual_api_step
    assert "-e MIMISEE_SMOKE_PASSWORD" in dual_api_step
...
def test_main_ci_runs_core_e2e_against_the_existing_host_backend() -> None:
    ...
    assert 'NEXT_PUBLIC_API_URL="http://127.0.0.1:${MIMISEE_RETRIEVAL_API_PORT}"' in regression_job
    ...
    assert 'CORE_E2E_BASE_URL="http://127.0.0.1:${MIMISEE_RETRIEVAL_API_PORT}"' in regression_job
...
def test_host_regression_jobs_isolate_database_and_api_ports() -> None:
    ...
    for job, port_var in (
        (retrieval_job, "MIMISEE_RETRIEVAL_API_PORT"),
        (kg_job, "MIMISEE_KG_API_PORT"),
    ):
    ...
    assert 'echo "MIMISEE_RETRIEVAL_WEB_PORT=$port" >> "$GITHUB_ENV"' in retrieval_job
    assert 'PLAYWRIGHT_PORT="$MIMISEE_RETRIEVAL_WEB_PORT"' in retrieval_job
