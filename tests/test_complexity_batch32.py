...
def test_check_knowledge_map_preserves_failure_order_and_counts(
    ...
    def _resolve(plugin_ref: str) -> dict[str, object]:
        return {} if plugin_ref in {"plugin:武进区@1.0.0"} else {"schema": "mimisee.retrieval_policy.v1"}
    ...
    assert report["schema"] == "mimisee.changzhou_gov_service_knowledge.dify_knowledge_map_check.v1"
    ...
    assert http_patches == [
        {
            ...
            "http_node_title": "MimiSee HTTP检索",
            "payload_node_id": "178309900ode",
            "payload_node_title": "安全构造 MimiSee 检索请求",
            "knowledge_id": "kb-city",
        }
    ]
...
def test_build_readiness_summary_preserves_stage_order_and_warning_details() -> None:
    report = build_readiness_summary(
        ...
        mimisee_direct={
            ...
            "source": {"base_url": "http://mimisee.internal", "base_host": "mimisee.internal"},
        },
        ...
        external_probe={
            ...
            "source": {"endpoint_host": "mimisee.internal"},
...
def test_format_status_and_markdown_evidence_preserve_sections_and_order() -> None:
    report = {
        ...
        "external_probe": {
            "status": "passed",
            "endpoint_host": "mimisee.internal",
            "summary": {"dify_hit_nonempty": 1, "probe_errors": 0},
            "boundary": {"verdict": "reachable"},
        },
        "full_gate": {
    ...
    assert status_text.splitlines() == [
        ...
        "Root cause: mimisee_direct (quality_gate_failed:answer_grounding_rate)",
        ...
        "MimiSee direct base: http://mimisee.internal (matches external endpoint host)",
        (
...
def test_build_sharing_markdown_and_generate_only_keep_output_contracts(
    ...
    report = {
        ...
        "case_type_advantage": [{"case_type": "mixed", "system": "dify_http_mimisee", "business_score": 0.9}],
        "dimension_advantage": [{"dimension": "材料", "system": "dify_http_mimisee", "business_score": 0.9}],
        "advantage_summary": {
            "overall_best_system": "dify_http_mimisee",
    ...
    apps = [AppSpec(label="demo", app_id="app-1", kind="http_to_mimisee", description="Demo", api_key="secret")]
    ...
    assert apps_payload == {
        "schema": "mimisee.dify_3way_benchmark.apps.v1",
        "apps": [
            {
                "label": "demo",
                "app_id": "app-1",
    ...
    assert cases_payload["schema"] == "mimisee.dify_3way_benchmark.cases.v1"
    assert cases_payload["summary"]["cases"] == 1
    assert truth_payload["schema"] == "mimisee.dify_3way_benchmark.truth_manifest.v1"
