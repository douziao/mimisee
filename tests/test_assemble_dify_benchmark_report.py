...
def test_merge_run_payloads_combines_items_by_case_id() -> None:
    ...
    merged = mod.merge_run_payloads([first, second], run_name="run_dify_http_mimisee.json")

    assert merged["system"] == "dify_http_mimisee"
...
def test_merge_run_payloads_uses_latest_payload_metadata() -> None:
    ...
    first = {
        "system": "dify_external_mimisee",
        "app": {"label": "dify_external_mimisee", "mode": "chat"},
    ...
    second = {
        "system": "dify_external_mimisee",
        "app": {"label": "dify_external_mimisee", "mode": "workflow"},
        "source": {"endpoint": "https://new.example/v1"},
        "summary": {"cases": 2},
        "items": [{"case_id": "case-1", "ok": True, "answer": "ok"}],
    }

    merged = mod.merge_run_payloads([first, second], run_name="run_dify_external_mimisee.json")
