...
def _postgres_alembic_test_database(monkeypatch: pytest.MonkeyPatch):
    ...
    db_name = f"mimisee_alembic_{uuid.uuid4().hex[:12]}"
...
@pytest.mark.skipif(not _integration_enabled(), reason="Integration tests disabled (set MIMISEE_INTEGRATION_TESTS=1)")
