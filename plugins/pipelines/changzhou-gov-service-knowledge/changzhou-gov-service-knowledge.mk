...
CHANGZHOU_DIFY_MIMISEE_BASE_URL ?= http://127.0.0.1:8000
...
CHANGZHOU_DIFY_MIMISEE_ENV_FILE ?= .env
CHANGZHOU_DIFY_MIMISEE_DIRECT_OUT ?= /tmp/changzhou_gov_dify_mimisee_direct_gate.json
CHANGZHOU_DIFY_MIMISEE_DIRECT_EXTRA_ARGS ?= --quality-profile changzhou-retrieval --min-hit-at-1 1 --min-answer-grounding-rate 1 --min-answer-key-point-recall 1
CHANGZHOU_DIFY_MIMISEE_DIRECT_KG_OFF_OUT ?= /tmp/changzhou_gov_dify_mimisee_direct_kg_off.json
CHANGZHOU_DIFY_MIMISEE_DIRECT_KG_ON_OUT ?= /tmp/changzhou_gov_dify_mimisee_direct_kg_on.json
CHANGZHOU_DIFY_KG_BASELINE_REPORT ?= $(CHANGZHOU_DIFY_MIMISEE_DIRECT_KG_OFF_OUT)
CHANGZHOU_DIFY_KG_CANDIDATE_REPORT ?= $(CHANGZHOU_DIFY_MIMISEE_DIRECT_KG_ON_OUT)
...
changzhou-gov-plugin-help:
	...
	@echo "  make changzhou-dify-mimisee-direct-gate - run MimiSee-only Changzhou golden retrieval gate"
	@echo "  make changzhou-dify-kg-on-off-gate - run KG-off/KG-on MimiSee golden gates and compare reports"
	@echo "  make changzhou-dify-kg-compare-gate - compare saved KG-off/KG-on Changzhou golden reports"
	@echo "  make changzhou-dify-external-probe - compare Dify external hit-testing with direct MimiSee retrieval"
	...
	@echo "  make changzhou-dify-full-gate - run Changzhou Dify/MimiSee remote golden gate"
	@echo "  make changzhou-dify-readiness-gate - run external probe, full Dify/MimiSee gate, and write readiness summary"
	...
	@echo "  make changzhou-dify-4way-preflight - validate native/http/external/MimiSee-direct against the generated eval pack"
	...
	@echo "  make changzhou-kg-on-off-benchmark - compare local MimiSee direct results with KG off vs on on the same eval pack"
...
changzhou-gov-plugin-corpus-closed-loop-smoke:
	...
	$(PY) scripts/plugin_corpus_closed_loop_smoke.py \
		--base-url "$(CHANGZHOU_DIFY_MIMISEE_BASE_URL)" \
...
changzhou-dify-mimisee-direct-gate:
	$(PY) scripts/changzhou_gov_golden_eval.py \
		--cases "$(CHANGZHOU_DIFY_CASES)" \
		--base-url "$(CHANGZHOU_DIFY_MIMISEE_BASE_URL)" \
		--env-file "$(CHANGZHOU_DIFY_MIMISEE_ENV_FILE)" \
		--top-k $(CHANGZHOU_DIFY_PROBE_TOP_K) \
		--timeout $(CHANGZHOU_DIFY_PROBE_TIMEOUT) \
		--out "$(CHANGZHOU_DIFY_MIMISEE_DIRECT_OUT)" \
		$(CHANGZHOU_DIFY_MIMISEE_DIRECT_EXTRA_ARGS)

changzhou-dify-mimisee-direct-kg-off-gate:
	$(PY) scripts/changzhou_gov_golden_eval.py \
		--cases "$(CHANGZHOU_DIFY_CASES)" \
		--base-url "$(CHANGZHOU_DIFY_MIMISEE_BASE_URL)" \
		--env-file "$(CHANGZHOU_DIFY_MIMISEE_ENV_FILE)" \
		...
		--timeout $(CHANGZHOU_DIFY_PROBE_TIMEOUT) \
		--kg-mode off \
		--out "$(CHANGZHOU_DIFY_MIMISEE_DIRECT_KG_OFF_OUT)" \
		$(CHANGZHOU_DIFY_MIMISEE_DIRECT_EXTRA_ARGS)

changzhou-dify-mimisee-direct-kg-on-gate:
	$(PY) scripts/changzhou_gov_golden_eval.py \
		--cases "$(CHANGZHOU_DIFY_CASES)" \
		--base-url "$(CHANGZHOU_DIFY_MIMISEE_BASE_URL)" \
		--env-file "$(CHANGZHOU_DIFY_MIMISEE_ENV_FILE)" \
		...
		--timeout $(CHANGZHOU_DIFY_PROBE_TIMEOUT) \
		--kg-mode on \
		--out "$(CHANGZHOU_DIFY_MIMISEE_DIRECT_KG_ON_OUT)" \
		$(CHANGZHOU_DIFY_MIMISEE_DIRECT_EXTRA_ARGS)
...
changzhou-dify-kg-on-off-gate:
	$(MAKE) changzhou-dify-mimisee-direct-kg-off-gate
	$(MAKE) changzhou-dify-mimisee-direct-kg-on-gate
...
changzhou-dify-readiness-gate:
	...
	rm -f "$(CHANGZHOU_DIFY_PROBE_OUT)" "$(CHANGZHOU_DIFY_OUT_PREFIX).json" "$(CHANGZHOU_DIFY_OUT_PREFIX)_answers.json" \
		...
		"$(CHANGZHOU_DIFY_KNOWLEDGE_MAP_OUT)" "$(CHANGZHOU_DIFY_MIMISEE_DIRECT_OUT)" "$(DIFY_CONSOLE_CHECK_OUT)"; \
	$(MAKE) changzhou-dify-knowledge-map-check; map_rc=$$?; \
	if [ $$map_rc -eq 0 ]; then \
		$(MAKE) changzhou-dify-mimisee-direct-gate; direct_rc=$$?; \
	else \
		direct_rc=1; \
	fi; \
...
changzhou-dify-readiness-summary:
	$(PY) scripts/changzhou_gov_dify_readiness_summary.py \
		...
		--mimisee-direct "$(CHANGZHOU_DIFY_MIMISEE_DIRECT_OUT)" \
...
changzhou-dify-readiness-persist-audit:
	...
	$(PY) scripts/persist_retrieval_audit_snapshot.py \
		...
		--base-url "$(CHANGZHOU_DIFY_MIMISEE_BASE_URL)" \
		--dataset-id "$(CHANGZHOU_GOV_CORPUS_DATASET_ID)" \
		--tenant-id "$(MIMISEE_TENANT_ID)" \
		--account-id "$(MIMISEE_ACCOUNT_ID)" \
		--user-id "$(MIMISEE_USER_ID)" \
		$(if $(strip $(MIMISEE_API_TOKEN)),--bearer "$(MIMISEE_API_TOKEN)") \
		--timeout $(MIMISEE_API_TIMEOUT) \
		--verify-report \
...
changzhou-dify-full-gate:
	$(PY) scripts/changzhou_gov_dify_full_gate.py \
		...
		--storage-state "$(CHANGZHOU_DIFY_STORAGE_STATE)" \
		--mimisee-base-url "$(CHANGZHOU_DIFY_MIMISEE_BASE_URL)" \
...
changzhou-dify-4way-preflight:
	$(PY) scripts/dify_3way_benchmark.py \
		...
		--app 'dify_http_mimisee:00000000-0000-0000-0000-000000000002:http_to_mimisee:dify_http_mimisee:auto' \
		--app 'dify_external_mimisee:00000000-0000-0000-0000-000000000003:external_to_mimisee:dify_external_mimisee:auto' \
		--include-mimisee-direct \
		--auto-mode \
		--timeout 120 \
		--preflight
