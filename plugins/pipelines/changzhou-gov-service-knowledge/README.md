...
MimiSee 的 Dify external knowledge adapter 支持平台通用的 `knowledge_id -> dataset_ids`
...
固定工作流的区县参数提取可能失败，查询里出现明确区县词时，MimiSee 会先查对应区县
数据集，再查本级兜底数据集：
...
导出的 JSON 是 `mimisee.regression_cases.v1`，但带有 `review_only=true`。
...
答案采集，不改变 MimiSee 检索评估。

只评估 MimiSee Dify external knowledge 检索与证据可回答性：

```bash
make changzhou-dify-mimisee-direct-gate
```

该命令只直打 MimiSee `/api/v1/integrations/dify/retrieval`，不依赖 Dify Console
登录态；token 默认从 `.env` 的 `DIFY_EXTERNAL_KNOWLEDGE_API_KEY` 或
`DIFY_EXTERNAL_KNOWLEDGE_API_KEYS` 读取，不会出现在命令行。默认输出
`/tmp/changzhou_gov_dify_mimisee_direct_gate.json`。
...
MimiSee Dify external knowledge adapter 会在返回给 Dify 的 records 里临时前置
...
标题已经由 MimiSee `query_routes` 纠偏到目标区县，则计入 `route_compensated_cases`，
...
`missing_variable=areaName`，失败边界在 Dify workflow 入参，不在 MimiSee 检索。
...
变量时，Dify 会直接返回 `HTTP 400 invalid_param`，请求不会进入 MimiSee 检索链路。
...
MimiSee 直查 golden eval -> Dify workflow trace。
命令需要 `DIFY_EXTERNAL_KNOWLEDGE_API_KEY` 环境变量，或显式传入 `--mimisee-token`。
...
python scripts/changzhou_gov_dify_full_gate.py \
  ...
  --storage-state /tmp/dify_console_storage_state.json \
  --mimisee-base-url http://127.0.0.1:8000 \
...
调用 MimiSee 失败”，用同一套 fixed golden cases 做边界对照：
...
再分别调用 Dify dataset `external-hit-testing` 与 MimiSee `/retrieval` 直查。
报告只保留 endpoint、dataset、命中数量和首条标题，不输出或保存 api key。
`boundary.verdict=dify_external_boundary_ok` 表示 endpoint 配置非 loopback、
本机直连 MimiSee 正常、Dify dataset hit-testing 也正常；这时如果 workflow 仍空召回，
...
`dify_runtime_empty_but_mimisee_direct_ok` 表示同一个 case 在 Dify 数据集召回测试为空，
但同一 endpoint/key 直打 MimiSee 有结果，失败边界在 Dify external knowledge runtime
到 MimiSee endpoint 之间，通常优先检查 Dify worker/container 的网络、代理与
`NO_PROXY` 配置。

如果 `dify_hit_nonempty > 0` 且 `mimisee_direct_nonempty > 0`，但 workflow trace
仍然显示 `empty_retrieval_cases > 0`，失败边界已经不在 MimiSee endpoint。
...
curl --noproxy '*' -sS \
  ...
  http://mimisee.example:8000/api/v1/integrations/dify/retrieval
```

如果 `--noproxy '*'` 有结果，但 Dify 页面/Console hit-testing 仍为空，
需要把 Dify api/worker 的 `NO_PROXY/no_proxy` 加上 `mimisee.example`
并重启 Dify 后端运行进程。若容器内 curl 本身无法连通，则需要给 Dify 所在主机或容器
提供能访问 MimiSee 的地址（同网段 LAN 地址、反向代理地址或部署后的 MimiSee 服务地址）。

当前远程 Dify 固定 workflow 曾经的失败模式是：MimiSee golden 检索 gate 通过，
...
MimiSee external dataset 的检索参数后，节点 trace 恢复为非空，兜底率降为 0。
