# MimiSee 提示词全集（业界采集 + 现状基线中文化）

> **本文档定位**：MimiSee 系统所需的全部 prompt 提示词的中文参考手册。**上半部分（Part A）** 是业界调研对比：5 大风格速查、11+ 开源 prompt 库索引、MimiSee 现状速查；**下半部分（Part B）** 是按 13 类场景给出的可直接复用中文 prompt 库——每类至少含"现状基线（MimiSee 现有 prompt 中文转写）"+"业界优秀版本（中文翻译并适配）"两条，共 26+ 个完整 prompt。
>
> **注意**：
...
- [A.3 MimiSee 当前 prompt 分布速查](#a3-mimisee-当前-prompt-分布速查)
...
**MimiSee 使用位置**：`app/rag/engine.py:234-263`（英文，含安全规则 6 条 + 回答要求 6 条 + 引用要求）
...
> 来源：MimiSee `app/rag/engine.py:234-263`（英文原版）
...
> 来源：MimiSee `app/rag/engine.py:270-282`
...
> 来源：MimiSee `app/rag/engine.py:288-298`
...
> 来源：MimiSee `app/rag/engine.py:303-312`
...
> 来源：MimiSee `app/rag/engine.py:317-326`
...
> 来源：MimiSee `app/rag/engine.py:332-342`
...
**MimiSee 使用位置**：`app/rag/reranker/llm_based.py:248`
...
> 来源：MimiSee `app/rag/reranker/llm_based.py:248`
...
> 来源：MimiSee `app/rag/kg/extraction/processor.py:100-109`
...
> 来源：MimiSee `app/rag/kg/extraction/entity_verifier.py:129-150`
...
> 来源：MimiSee `app/rag/kg/community.py:482-491`
...
> 来源：MimiSee `app/rag/llm/prompts/tagger_prompts.py:5`（**原文已中文**）
...
**场景定义**：检测 prompt injection、jailbreak、敏感信息泄露、有害内容。**当前 MimiSee 全部用规则 (regex)**;本节补足 LLM-as-classifier 路径。
...
> 来源：MimiSee `app/rag/evaluation/agent_evals.py:217`
...
#### 现状基线（MimiSee `agent_evals.py:311` 转写）
...
> 来源：MimiSee `app/rag/evaluation/agent_evals.py:311`
...
> 来源：MimiSee `app/rag/evaluation/ragas.py:611`
...
**MimiSee 使用位置**：`app/rag/workflows/evaluator_optimizer.py:177-199`
...
> 来源：MimiSee `app/rag/workflows/evaluator_optimizer.py:177`
...
**MimiSee 使用位置**：`app/rag/memory/short_term.py:244`
...
> 来源：MimiSee `app/rag/memory/short_term.py:244`
...
> 来源：MimiSee `app/api/v1/pipeline.py:2925`
...
**MimiSee 使用位置**：`app/rag/evaluation/test_generator.py:40,70`
...
> 来源：MimiSee `app/rag/evaluation/test_generator.py:40,70`（综合提炼）
...
> 来源：MimiSee `app/rag/agents/prebuilt.py:103-116`
...
```bash
# 1. 文件存在且行数合规
wc -l docs/prompts/mimisee-prompt-library-2026-q2.md
# 期望: 800-1200 行

# 2. 13 类场景每类有现状 + 业界
grep -c "^#### 现状基线" docs/prompts/mimisee-prompt-library-2026-q2.md
# 期望 ≥ 13

grep -c "^#### 业界优秀版本" docs/prompts/mimisee-prompt-library-2026-q2.md
# 期望 ≥ 13

# 3. 来源标注覆盖
grep -c "^> 来源" docs/prompts/mimisee-prompt-library-2026-q2.md
# 期望 ≥ 26

# 4. 变量占位符使用统一
grep -c "{{" docs/prompts/mimisee-prompt-library-2026-q2.md
# 期望大量 (~50+)
...
本手册为 MimiSee 提示词工程的**单一参考源 (single source of truth)**。任何新增 prompt 应:
