...
- 在线交互文档（GitHub Pages）：`https://douziao.github.io/mimisee/`（fork 请改为 `https://<owner>.github.io/<repo>/`）。
- **全栈手册（Docusaurus，可搜索）**：`https://douziao.github.io/mimisee/handbook/`（与 Redoc 同域；源码在仓库 `docs-site/`）。
...
- [architecture.md](./architecture.md)：MimiSee 技术架构总览。
...
- [guides/evidence_capsule.md](./guides/evidence_capsule.md)：Evidence Capsule 契约（`mimisee.evidence_capsule.v1`）。
...
- [benchmarks/changzhou_dify.md](./benchmarks/changzhou_dify.md)：常州政务 800 题 Dify / MimiSee 评测记录（方法、指标与历史复测）。
...
- [prompts/mimisee-prompt-library-2026-q2.md](./prompts/mimisee-prompt-library-2026-q2.md)：MimiSee 提示词全集（业界采集 + 现状基线中文化）。
