...
This folder contains **copy-pasteable** operational templates for MimiSee:

- `prometheus-rule-mimisee.yaml`: a `PrometheusRule` (Kubernetes / Prometheus Operator) with baseline alerts
- `grafana-dashboard-mimisee.json`: a Grafana dashboard JSON for an “Ops Overview” view
...
2. Prometheus scrape configured:
   - For Kubernetes: `deploy/helm/mimisee/templates/servicemonitor.yaml` (requires Prometheus Operator)

## What’s covered

Alerts and dashboards are aligned to the built-in metrics shipped by MimiSee:
...
If you deploy MimiSee via the built-in Helm chart (`deploy/helm/mimisee`), you can enable these templates via values:
...
```bash
kubectl apply -f docs/ops/templates/prometheus-rule-mimisee.yaml
```
...
2. Upload `docs/ops/templates/grafana-dashboard-mimisee.json`
...
- Default alert thresholds are conservative and workload-dependent:
  - `MimiSeeHighHttp429Ratio`: 429 ratio > 2% over 10m, only when overall traffic > 1 rps (sustained 15m).
  - `MimiSeeAuthzGroupDenyRatioHigh`: group deny ratio > 20% over 10m (sustained 20m).
  - `MimiSeeAuthzGroupDenySpike`: > 200 denies over 5m (sustained 15m).
