...
本指南用于把 MimiSee 的核心数据做成可演练的备份/恢复闭环，覆盖：
...
export PGDATABASE="mimisee"

mkdir -p backups/postgres
pg_dump -Fc -Z 6 -f "backups/postgres/mimisee.$(date -u +%Y%m%dT%H%M%SZ).dump"
```
...
```bash
export DUMP_FILE="backups/postgres/mimisee.<timestamp>.dump"

# 先建库（按你们规范）
createdb -h "$PGHOST" -p "$PGPORT" -U "$PGUSER" mimisee

# 再恢复（可加 --clean 覆盖旧对象；生产慎用）
pg_restore -h "$PGHOST" -p "$PGPORT" -U "$PGUSER" -d mimisee --no-owner --no-privileges "$DUMP_FILE"
```
...
```bash
mc alias set mimisee-minio https://<minio-endpoint> <access_key> <secret_key>

mkdir -p backups/minio
mc mirror --overwrite --remove --md5 mimisee-minio/<bucket-name> backups/minio/<bucket-name>
```
...
```bash
mc mirror --overwrite backups/minio/<bucket-name> mimisee-minio/<bucket-name>
```
...
MimiSee 运行依赖向量库实现检索。向量库数据是否“必须备份”，取决于：
...
> DR drill 的更完整流程与自动化脚本：见 `MimiSee-eh26.39` 任务产出（恢复验证 checklist + automation）。
