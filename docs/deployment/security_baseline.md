# Security Baseline（MimiSee / Kubernetes）

本页是面向平台/运维团队的 **K8s 安全基线落地指南**，目标是让 MimiSee 的私有化部署：
...
```bash
helm get values mimisee -n <ns>
kubectl -n <ns> get deploy,cronjob,networkpolicy,sa -l app.kubernetes.io/instance=mimisee
```
