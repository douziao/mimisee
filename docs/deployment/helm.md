# Helm / Kubernetes 部署指南（MimiSee）

本指南提供一个 **最小可用** 的 Helm Chart，用于在 Kubernetes 中部署：

- `mimisee-api`（FastAPI 服务）
- `mimisee-worker`（Arq 后台 worker）

本 Chart **默认不安装数据库/向量库/Redis/MinIO**，而是以“企业常见做法”对齐：依赖外部基础设施（自建或云托管）。

Chart 目录：`deploy/helm/mimisee`
...
Chart 默认使用 `mimisee/mimisee:<tag>`，你需要替换为自己的镜像仓库：

```bash
docker build -f docker/Dockerfile -t <your-registry>/mimisee:<tag> .
docker push <your-registry>/mimisee:<tag>
```
...
```bash
kubectl create namespace mimisee

kubectl -n mimisee create secret generic mimisee-env \
  --from-literal=ENV=production \
  --from-literal=AUTH_MODE=jwt \
  --from-literal=SECRET_KEY="<>=32 chars random>" \
  --from-literal=JWT_TENANT_CLAIM="tenant_id" \
  --from-literal=CORS_ORIGINS="https://mimisee.example.com" \
  --from-literal=DATABASE_URL="postgresql://user:pass@postgres:5432/mimisee" \
...
```yaml
image:
  repository: <your-registry>/mimisee
  tag: "<tag>"

existingSecretName: mimisee-env

ingress:
  enabled: true
  className: nginx
  hosts:
    - host: mimisee.example.com
      paths:
        - path: /
          pathType: Prefix
  tls:
    - secretName: mimisee-example-com-tls
      hosts:
        - mimisee.example.com
...
- `deploy/helm/mimisee/examples/values-prod.yaml`
- `deploy/helm/mimisee/examples/values-hardened.yaml`
...
- 完整示例文件：`deploy/helm/mimisee/examples/values-periodic-audits.yaml`
...
```bash
kubectl -n mimisee get pods
kubectl -n mimisee get svc
kubectl -n mimisee logs deploy/mimisee-mimisee-api -f
kubectl -n mimisee logs deploy/mimisee-mimisee-worker -f
```
...
```bash
kubectl -n mimisee port-forward svc/mimisee-mimisee 8000:8000
