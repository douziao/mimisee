# MimiSee HTTP API 参考（导读）

本目录提供 **人类可读** 的 API 导览；**完整路径、请求/响应模型** 以 OpenAPI 为准。

| 资源 | 说明 |
|------|------|
| **在线交互文档（GitHub Pages）** | 部署成功后访问：`https://douziao.github.io/mimisee/`（fork 后请改为 `https://<owner>.github.io/<repo>/`） |
| **全栈手册（Docusaurus）** | 同域：`https://douziao.github.io/mimisee/handbook/`（叙事 + 分区侧栏 + 搜索；与 Redoc 分工） |
