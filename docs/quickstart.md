...
docker compose --project-name mimisee --env-file .env -f docker/docker-compose.yml up -d --build
docker compose --project-name mimisee --env-file .env -f docker/docker-compose.yml ps
...
在页面完成首次设置后，将同一账号配置为 `MIMISEE_SMOKE_IDENTIFIER` 与 `MIMISEE_SMOKE_PASSWORD`，再运行 `make core-e2e`。
...
> Docker 前端默认通过同源 `/api/*` 代理访问后端；浏览器地址由 `NEXT_PUBLIC_API_URL_DOCKER=/` 控制，SSR 容器内地址由 `API_INTERNAL_URL_DOCKER=http://mimisee-api:8000` 控制。`make up-web` 会启动完整 Docker Web 栈，不是“只起前端”；本地热更新前端请使用 `make web`。不要把 Docker 内部主机名暴露给浏览器。

以下解析器都是可选 profile，默认不会启动。请按文档类型选择一个；完整选择矩阵、资源要求和最小 `.env` 配置见根目录 [README](../README.md#可选按文档类型启用解析器)。

### (可选) 启用 Docling Serve（独立 CPU 容器）

Docling 的 Python 依赖、PyTorch 和模型权重不安装进 MimiSee API / worker 镜像。MimiSee
...
全 Docker 部署也可显式写为 `DOCLING_API_URL=http://mimisee-docling:5001`。内置 profile
仅绑定宿主机 `127.0.0.1`，默认不启用 API Key；连接另行部署且已启用鉴权的服务时，
配置 `DOCLING_API_KEY`，MimiSee 会使用官方 `X-Api-Key` 请求头。该 profile 固定官方
CPU 镜像版本，并在启动时预热默认模型；镜像较大，但不会增加 MimiSee 主镜像及本地
Python venv 的依赖体积。
...
`http://mimisee-docling:5001`（宿主机仍用 `http://127.0.0.1:5001`）。CPU 与 GPU profile
...
MimiSee 已内置 `etl4llm` 解析器（并兼容 `bisheng` / `bisheng-unstructured` 别名），你只需要把服务跑起来并配置好 API URL：
...
ETL4LLM_API_URL=http://mimisee-etl4llm:10001/v1/etl4llm/predict
```

### (可选) 启用 Marker（启发式服务 PDF→Markdown）

Marker 建议以独立容器/服务运行（重依赖不进入 MimiSee 主镜像），然后在解析时指定 `parser_backend=marker`。
...
```env
MARKER_ENABLED=true
MARKER_API_URL=http://mimisee-marker:2080/convert
```

### (可选) 启用 PaddleOCR-VL（外部 OCR/版面解析）

PaddleOCR-VL 建议以独立容器/服务运行（重依赖不进入 MimiSee 主镜像），然后在解析时指定 `parser_backend=paddle_vl`。
...
PADDLE_VL_API_URL=http://mimisee-paddlevl:9030/convert
PADDLEOCR_DEVICE=gpu:0
```

### (可选) 启用 Qianfan-OCR（外部 OCR）

Qianfan-OCR 建议以独立容器/服务运行，MimiSee 通过包装服务调用上游 OpenAI-compatible 视觉推理接口。
...
QIANFAN_OCR_API_URL=http://mimisee-qianfanocr:2090/convert
# 包装服务上游（百度千帆在线 OCR）
...
文档解析能力接入到 MimiSee，而无需自建重型 OCR/版面服务。
...
MinerU 建议以独立容器运行（依赖/模型较重），MimiSee 通过 HTTP 调用其 `/file_parse` 接口拿到 ZIP（Markdown + images）。
...
MINERU_LOCAL_SERVER_URL=http://mimisee-mineru:8000
```
...
生产部署建议用 `mimisee-magicpdf` 独立服务，不再依赖 API / worker 镜像内的本地
`magic-pdf` CLI。服务会复用 `mineru_cache` 里的 PDF-Extract-Kit 模型缓存。

```env
MAGIC_PDF_ENABLED=true
MAGIC_PDF_API_URL=http://mimisee-magicpdf:2095/convert
...
```bash
make up-magicpdf
docker compose -f docker/docker-compose.yml exec -T -w /app mimisee-api python scripts/check_parsers.py
```
...
| `magicpdf` | 独立 `mimisee-magicpdf` 服务 + PDF-Extract-Kit 缓存模型 | CUDA 模式需按真实 PDF 单独量测 | 建议独立容器运行并设置 `MAGIC_PDF_DEVICE_MODE=cuda`；本地 CLI 仅作为调试兜底 |
...
MimiSee 通过 **OpenAI 兼容接口** 统一接入所有 LLM，**无需改代码**。将 `LLM_API_BASE` 指向目标模型的 OpenAI 兼容端点（自建网关或第三方兼容服务），并设置 `LLM_MODEL` / `LLM_API_KEY` 即可：
...
:::note
MimiSee 不内置 native Anthropic / Gemini SDK，所有模型经 OpenAI 兼容协议接入。若目标服务无兼容端点，需在前置网关（如 LiteLLM、One-API）做协议转换。
:::
...
2. 查看后端日志: `docker compose logs -f mimisee-api`
3. 访问 API 文档: http://localhost:8000/docs
4. 提交 Issue: [GitHub Issues](https://github.com/douziao/mimisee/issues)
