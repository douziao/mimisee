...
This document explains how MimiSee's chat/RAG pipeline is structured today and how it relates to the earlier "agent-style" approach.

## Current State (Recommended)

MimiSee keeps the RAG pipeline explicit and debuggable:
...
In addition, MimiSee exposes a **workflow mode** concept for advanced orchestration features (see `WORKFLOW_MODE` in `app/core/config.py`).
