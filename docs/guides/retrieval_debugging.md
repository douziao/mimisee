...
proof 语义（`mimisee.must_recall_proof.v1`）：

- `status/passed`：合同最终状态（含 second-pass 之后）。
- `obligation_ledger`：`mimisee.recall_obligation_ledger.v1`，明确 required/matched/missing。
...
`/api/v1/rag/retrieve` 现在可返回 `evidence_capsule`（`mimisee.evidence_capsule.v1`），用于不可变回放与审计归档：
