...
The report (`mimisee.parser_benchmark.v1`) includes:
...
- `--strict-profile ci/parser_strict_profile.v1.json`
- schema: `mimisee.parser_benchmark_strict_profile.v1`
- fields:
...
`parser_benchmark` now also emits `regression_severity` (schema `mimisee.parser_benchmark_regression_severity.v1`) when baseline is provided, including:
