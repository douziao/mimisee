...
MimiSee now supports a production-oriented SAML 2.0 ACS flow that terminates in the same app JWT/session model used by password login.
...
3. The backend validates the assertion and issues a normal MimiSee `AuthResponse`.
...
This keeps browser-facing SAML ingress on the web origin while preserving the backend as the only component that maps identities and signs MimiSee JWTs.
...
- Existing local MimiSee account resolution from `email` or `NameID`
