...
> 导出的 bundle schema 为 `mimisee.regression_cases.v1`，包含 `dataset_id` 与 `items[]`。
>
...
说明：
- `--cases` 支持两种格式：
  - bundle：`{"schema":"mimisee.regression_cases.v1","dataset_id":"...","items":[...]}`
...
你可以用一次 regression run 的 summary 直接生成 `mimisee.thresholds.v2`（包含 top-level + per-slice），用于后续 CI gate：
...
export DATABASE_URL="postgresql://postgres:postgres@localhost:5432/mimisee"
...
EMBEDDING_PROVIDER=deterministic_test EMBEDDING_MODEL=mimisee-deterministic-test-v1 \
