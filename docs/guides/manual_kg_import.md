...
MimiSee supports importing a manually governed knowledge graph into the existing
KG tables. Domain-specific extraction and alignment should happen outside the
core application; MimiSee only validates, stores, scopes, queries, and rolls back
curated graph rows.
