...
更主流、也更可复现的做法是：
- **统一评测格式**（`mimisee.regression_cases.v1`）
...
- 在 MimiSee 中我们把 wiki 页面切成 “sentence-per-chunk”，再把 evidence 指针对齐到 `reference_sources.chunk_id`
...
- 如果你希望 nightly 结果更可复现，建议传 `--cases <mimisee.regression_cases.v1 bundle>` 锁定 suite。  
...
1. **换 embedding 模型就要重建向量**  
   MimiSee 会在检索侧做 `embedding_space_hash` guard，避免不同 embedding space 混用导致 silent 退化。
