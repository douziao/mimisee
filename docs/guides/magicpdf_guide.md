...
MimiSee 支持将 **MagicPDF（PyPI: `magic-pdf`）** 作为可选 PDF 高级解析后端，输出 Markdown，适用于扫描件、复杂排版等场景。

> 生产部署使用独立 `mimisee-magicpdf` 服务。默认 API/worker 镜像不安装依赖版本与主运行时冲突的本地 CLI；本地 CLI 仅保留为宿主机或自定义镜像的开发调试路径。
...
`mimisee-magicpdf` 镜像使用 CUDA PyTorch 运行时；如果服务器有 NVIDIA GPU，
...
```bash
docker compose -f docker/docker-compose.yml up -d --build mimisee-api mimisee-worker
```
...
MAGIC_PDF_MODELS_DIR=   # 可选；留空时自动查找 /opt/mimisee-model-cache 和 Hugging Face 缓存
...
`docker/docker-compose.parsers.yml` 会把 `mineru_cache` 挂到 `mimisee-magicpdf` 的
`/opt/mimisee-model-cache:ro`。推荐先启动一次本地 MinerU 服务下载/填充 PDF-Extract-Kit 模型缓存，再启用 MagicPDF：
