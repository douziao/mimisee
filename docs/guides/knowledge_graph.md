...
MimiSee 的 Knowledge Graph（KG）以“事件（Event）—实体（Entity）”为核心结构：
...
返回是 `mimisee.kg_snapshot_diff.v1`：
```json
{
  "schema": "mimisee.kg_snapshot_diff.v1",
...
MimiSee 的 KG 默认不依赖图数据库，核心数据直接落在 PostgreSQL，并为向量召回额外写入 Milvus。
...
MimiSee 提供一个 **Dynamic OneEval 风格**的 KG search 诊断接口，用于提升抽取/搜索质量并可回归：
