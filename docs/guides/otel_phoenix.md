...
MimiSee 后端内置 **OpenTelemetry tracing**（默认关闭）。开启后可将 FastAPI 请求链路与 httpx 出站请求导出到 OTLP Collector（例如 Arize Phoenix）。
...
将后端 `OTEL_EXPORTER_OTLP_ENDPOINT` 指向 Phoenix 的 4317 端口后，启动 MimiSee，访问任意 API（如 `/docs` / `/api/v1/chat/stream`）即可在 Phoenix 中看到 traces。

## 3) 重要：Phoenix Cloud / 协议差异

当前 MimiSee 后端 exporter 使用 **OTLP gRPC**（`opentelemetry-exporter-otlp-proto-grpc`）。
