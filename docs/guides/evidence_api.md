...
```json
{
  ...
  "abstain_reason": null,
  "retrieval_trace": {
    "schema": "mimisee.retrieval_trace.v1",
    "selected_pass": "primary",
    "passes": [
      {
        "pass": "primary",
        "trace": {
          "schema": "mimisee.retrieval_trace_pass.v1",
          "retrieval_mode": "hybrid",
...
- `schema`: response schema identifier (currently `mimisee.evidence.v1`). Downstream consumers can use this to
...
Schema notes:
- `retrieval_trace.schema` is currently `mimisee.retrieval_trace.v1`.
- Each pass item contains a nested trace with `trace.schema = mimisee.retrieval_trace_pass.v1`.
...
- Each pass trace may include a `retrieval_config` fingerprint:
  - `retrieval_config.schema = mimisee.retrieval_config.v1`
