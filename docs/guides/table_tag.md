...
MimiSee 支持将结构化表格（CSV/XLS/XLSX）走 **TAG**（Table Augmented Generation）链路：把表格导入每文档 SQLite（Table Store），通过 **SQL 查询**（以及可选的 NL→SQL/语义过滤）来回答问题，而不是强行把大表切块嵌入进向量库。
...
> 说明：该接口路径保留了历史命名（`/lotus/sem-filter`），但实现为 MimiSee 内置的“LOTUS-like”语义过滤能力，不依赖外部 LOTUS 包。
