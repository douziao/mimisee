...
- Docker：`make logs` 或 `docker compose -f docker/docker-compose.yml logs -f mimisee-api`
...
> 不要把 `NEXT_PUBLIC_API_URL` 配成 `http://mimisee-api:8000`（这是 Docker 内部 DNS，浏览器不认识）。

### Docker 前端（SSR）需要容器内地址时
在 `.env` 配：
- `NEXT_PUBLIC_API_URL_DOCKER=/`
- `API_INTERNAL_URL_DOCKER=http://mimisee-api:8000`
