...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--base-url",
        default=(
            os.getenv("MIMISEE_BASE_URL")
            or os.getenv("MIMISEE_API_BASE_URL")
            or os.getenv("BACKEND_BASE_URL")
            or os.getenv("CHANGZHOU_DIFY_MIMISEE_BASE_URL")
            or "http://127.0.0.1:8000"
        ),
        help="MimiSee backend base URL; root or /api/v1 is accepted.",
    )
    ...
    parser.add_argument("--timeout", type=float, default=float(os.getenv("MIMISEE_API_TIMEOUT") or "60"))
    parser.add_argument(
        "--verify-report",
        action="store_true",
