...
PLUGIN_RELEASE_GATE_SCHEMA = "mimisee.plugin_release_gate.v1"
...
def build_plugin_release_gate_report(
    ...
    *,
    ...
    golden_declared = (descriptor.golden_rules or {}).get("schema") == "mimisee.golden_rules.v1"
...
def _parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run a generic MimiSee pipeline plugin release gate.")
    parser.add_argument(
        "--plugin-dir", required=True, help="Plugin package directory containing mimisee-plugin.json/yaml."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="Fail the gate if the plugin does not declare mimisee.retrieval_policy.v1.",
    )
