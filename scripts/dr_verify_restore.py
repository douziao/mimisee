...
def _run_smoke_test(
    *,
    ...
    with tempfile.TemporaryDirectory(prefix="mimisee-dr-") as tmp:
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--token", default=os.getenv("MIMISEE_DR_ADMIN_TOKEN", ""), help="Bearer token (AUTH_MODE=jwt).")
...
def _build_report(*, api_base: str, tenant_id: str) -> dict[str, Any]:
    return {
        "schema": "mimisee.dr_verify_restore.v1",
        "ran_at": _utc_now_iso(),
        "api_base": api_base,
        "tenant_id": tenant_id,
        "steps": {},
