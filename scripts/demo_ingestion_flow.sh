...
IDENTIFIER="${IDENTIFIER:-${MIMISEE_DEMO_IDENTIFIER:-}}"
PASSWORD="${PASSWORD:-${MIMISEE_DEMO_PASSWORD:-}}"
TOKEN="${TOKEN:-${MIMISEE_DEMO_TOKEN:-}}"
...
if [[ -z "$TOKEN" ]]; then
  if [[ -z "$IDENTIFIER" || -z "$PASSWORD" ]]; then
    echo "[demo] ERROR: provide TOKEN or IDENTIFIER+PASSWORD (or MIMISEE_DEMO_IDENTIFIER/MIMISEE_DEMO_PASSWORD)" >&2
    exit 1
  fi
  echo "[demo] Login..."
