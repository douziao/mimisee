#!/usr/bin/env python3
"""Run Dify/MimiSee app benchmarks against deterministic rubrics.
...
DEFAULT_MIMISEE_BASE_URL = "http://127.0.0.1:8000"
...
DEFAULT_APP_SPECS = [
    {
        "label": "dify_http_mimisee",
        "app_id": "00000000-0000-0000-0000-000000000002",
        "kind": "http_to_mimisee",
        "description": "Dify HTTP 节点调用 MimiSee",
    },
    {
...
def extract_mimisee_records_from_console_node_executions(payload: dict[str, Any]) -> list[dict[str, Any]]:
...
def lookup_dify_console_mimisee_records(
    *,
    app: AppSpec,
    item: dict[str, Any],
    ...
    try:
        ...
        records = extract_mimisee_records_from_console_node_executions(executions)
        return {
            "records": records,
...
def _query_mimisee_history_records_once(
    *,
    app: AppSpec,
    item: dict[str, Any],
    require_run_id: bool,
...
def lookup_mimisee_history_records(
    *,
    app: AppSpec,
    item: dict[str, Any],
    wait_sec: float,
    ...
    source_conversation_id = _history_source_conversation_id(app, item)
    if not source_conversation_id:
        return {"records": [], "source": "mimisee_history", "status": "missing_conversation_id"}
    ...
    while True:
        try:
            records = _query_mimisee_history_records_once(app=app, item=item, require_run_id=True)
            if not records:
                records = _query_mimisee_history_records_once(app=app, item=item, require_run_id=False)
            if records:
                return {
                    "records": records,
                    "source": "mimisee_history",
                    "status": "found",
...
def load_mimisee_token(explicit_token: str, *, env_file: str = ".env") -> str:
...
def load_app_specs(raw_specs: list[str], key_file: str) -> list[AppSpec]:
    ...
    return [
        AppSpec(
            ...
            kind=("http_to_mimisee" if _text(item.get("kind")) == "http_mimisee" else _text(item.get("kind"))),
...
def build_key_requirements(apps: list[AppSpec]) -> dict[str, Any]:
    ...
    return {
        "schema": "mimisee.dify_3way_benchmark.key_requirements.v1",
...
def resolve_app_modes(
    *,
    ...
    for app in apps:
        ...
        items.append(item)
    return resolved, {
        "schema": "mimisee.dify_3way_benchmark.mode_resolution.v1",
        "generated_at": _utc_now_text(),
...
def _call_case(
    *,
    ...
    try:
        ...
        if not item["records"] and app.kind == "http_to_mimisee" and history_records_fn is not None:
            ...
            if history_records:
                ...
                item["record_source"] = _text(history_meta.get("source")) or "mimisee_history"
        ...
        if not item["records"] and app.kind == "http_to_mimisee" and console_records_fn is not None:
...
def load_report_only_runs(*, out_dir: Path, apps: list[AppSpec], include_mimisee_direct: bool) -> list[dict[str, Any]]:
    runs: list[dict[str, Any]] = []
    if include_mimisee_direct:
        run = _load_run_file(out_dir / "run_mimisee_direct.json", system="mimisee_direct")
        if run is not None:
            runs.append(run)
    for app in apps:
...
def _missing_key_run_from_existing(
    *,
    ...
    out: dict[str, Any] = {
        "schema": "mimisee.dify_3way_benchmark.run.v1",
        "generated_at": _utc_now_text(),
        "system": system,
        "app": app_payload,
        "summary": summary,
    ...
    *,
    ...
    for app in apps:
        ...
        result = _call_case(
            ...
            history_records_fn=(
                (
                    lambda *, app, item, **_kwargs: lookup_mimisee_history_records(
                        app=app,
                        item=item,
                ...
                if http_history_backfill and app.kind == "http_to_mimisee"
                else None
            ),
            console_records_fn=(
                (
                    lambda *, app, item, timeout, **_kwargs: lookup_dify_console_mimisee_records(
                        app=app,
                        item=item,
                        console_base_url=console_base_url,
                        console_token=console_token,
                ...
                if console_backfill and _text(console_token) and app.kind == "http_to_mimisee"
                else None
            ),
        )
        item.update(
    ...
    return {
        "schema": "mimisee.dify_3way_benchmark.preflight.v1",
        "generated_at": _utc_now_text(),
        "base_url": base_url.rstrip("/"),
        "case": {
            "case_id": _case_id(probe_case),
...
def _history_records_lookup(
    *,
    ...
    if not enabled or app.kind != "http_to_mimisee":
        return None
    return lambda *, app, item, **_kwargs: lookup_mimisee_history_records(
        app=app,
        item=item,
        wait_sec=wait_sec,
        poll_sec=poll_sec,
...
def _console_records_lookup(
    *,
    ...
    if not enabled or not _text(console_token) or app.kind != "http_to_mimisee":
        return None
    return lambda *, app, item, timeout, **_kwargs: lookup_dify_console_mimisee_records(
        app=app,
        item=item,
        console_base_url=console_base_url,
        console_token=console_token,
    ...
    *,
    ...
    if timeout_case_ids:
        ...
        retry_results = _retry_timed_out_cases(
            ...
            retry_timeout=retry_timeout,
            concurrency=concurrency,
            worker=lambda case: _call_mimisee_case(
                case=case,
                base_url=base_url,
                token=token,
                timeout=retry_timeout,
...
def build_artifact_manifest(
    *, out_dir: Path, report: dict[str, Any], apps: list[AppSpec], include_mimisee_direct: bool
) -> dict[str, Any]:
    filenames = [
        "cases_800.json",
        "truth_manifest.json",
    ...
    if include_mimisee_direct:
        filenames.append("run_mimisee_direct.json")
    ...
    return {
        "schema": "mimisee.dify_3way_benchmark.artifact_manifest.v1",
        "generated_at": _utc_now_text(),
        "out_dir": str(out_dir),
...
def build_completion_status(
    *,
    ...
    include_mimisee_direct: bool = False,
) -> dict[str, Any]:
    required_systems = {app.label for app in apps}
    if include_mimisee_direct:
        required_systems.add("mimisee_direct")
...
def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Generate and run an evidence-first Dify/MimiSee benchmark.")
    ...
    parser.add_argument("--user-prefix", default="mimisee-dify-3way")
    parser.add_argument("--include-mimisee-direct", action="store_true")
    parser.add_argument("--mimisee-base-url", default=DEFAULT_MIMISEE_BASE_URL)
    parser.add_argument("--mimisee-token", default="")
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="Do not backfill HTTP_mimisee evidence from locally ingested MimiSee history citations.",
    )
    parser.add_argument(
        "--http-history-backfill-wait-sec",
        type=float,
        default=3.0,
        help="Max seconds to wait for async MimiSee history ingest after each HTTP_mimisee Dify response.",
    )
    parser.add_argument(
        "--http-history-backfill-poll-sec",
        type=float,
        default=0.5,
        help="Polling interval for HTTP_mimisee history evidence backfill.",
    )
    parser.add_argument(
        "--no-dify-console-backfill",
        action="store_true",
        help="Do not use Dify Console node executions to backfill HTTP_mimisee evidence.",
    )
    parser.add_argument(
...
def _apps_manifest(apps: list[AppSpec]) -> dict[str, Any]:
    return {"schema": "mimisee.dify_3way_benchmark.apps.v1", "apps": _redacted_apps(apps)}
...
def _write_main_inputs(
    *,
    ...
    _write_json(
        ...
        {
            "schema": "mimisee.dify_3way_benchmark.cases.v1",
    ...
    _write_json(
        ...
        {
            "schema": "mimisee.dify_3way_benchmark.truth_manifest.v1",
            "summary": summarize_cases(cases),
    ...
    *,
    ...
    if args.report_only:
        return load_report_only_runs(
            out_dir=out_dir,
            apps=apps,
            include_mimisee_direct=bool(args.include_mimisee_direct),
        )
    if args.include_mimisee_direct:
...
def _append_main_direct_run(
    *,
    ...
    direct_run_path = out_dir / "run_mimisee_direct.json"
    run = run_mimisee_direct(
        cases=cases_to_run,
        base_url=str(args.mimisee_base_url),
        token=load_mimisee_token(str(args.mimisee_token), env_file=str(args.env_file)),
        timeout=float(args.timeout),
...
def _empty_quality_report(cases_to_run: list[dict[str, Any]]) -> dict[str, Any]:
    return {
        "schema": "mimisee.mixed_rag_quality_report.v1",
        "generated_at": _utc_now_text(),
