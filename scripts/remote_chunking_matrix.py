...
def maybe_copy_or_download_long_pdf(target_dir: Path) -> Path:
    ...
    configured_fixture = str(os.getenv("MIMISEE_REMOTE_LONG_PDF_FIXTURE", "") or "").strip()
    if configured_fixture:
        source = Path(configured_fixture).expanduser()
        if not source.is_file():
            raise FileNotFoundError(f"MIMISEE_REMOTE_LONG_PDF_FIXTURE does not exist: {source}")
        shutil.copy2(source, target)
        return target
    if str(os.getenv("MIMISEE_REMOTE_FIXTURE_DOWNLOADS", "") or "").strip().lower() not in {"1", "true", "yes", "on"}:
        shutil.copy2(SMALL_PDF_FIXTURE, target)
        return target
    from urllib.request import Request, urlopen

    request = Request(LONG_PDF_URL, headers={"User-Agent": "MimiSee remote chunking matrix"})
...
def prepare_fixture_files(fixtures_dir: Path) -> list[dict[str, Any]]:
    ...
    markdown_path.write_text(
        "# Rollout Handbook\n\n"
        "MimiSee rollout notes explain parser service health, retrieval tuning, and governance cleanup.\n\n"
        "## Parser Service\n\n"
    ...
    html_path.write_text(
        ...
        "<p>MimiSee tracks parser health, dataset coverage, and audit evidence.</p>"
