...
def _parse_args(argv: list[str] | None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run the MimiSee performance harness suite.")
    ...
    parser.add_argument(
        ...
        help="Base URL for the MimiSee API (default: http://localhost:8000).",
    )
    parser.add_argument(
        "--llm-mock",
    ...
    parser.add_argument(
        "--query",
        default="What is MimiSee?",
        help="Query string for retrieval-preview case (default: 'What is MimiSee?').",
    )
...
def _build_headers(*, tenant_id: str, user_id: str, token: str) -> dict[str, str]:
    headers: dict[str, str] = {"User-Agent": "mimisee-perf-harness/1.0"}
    if tenant_id:
    ...
    if dataset_id and headers.get("X-Tenant-ID") and (headers.get("Authorization") or headers.get("X-User-ID")):
        cases.append(
            PerfCase(
                ...
                json_body={
                    "query": str(getattr(args, "query", "") or "What is MimiSee?"),
