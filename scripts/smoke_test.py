...
def _bootstrap_register_enabled(
    ...
    return bool(args.bootstrap_register or _is_truthy(env_or(dotenv, "MIMISEE_SMOKE_BOOTSTRAP_REGISTER", "")))
...
def _resolve_jwt_token(
    ...
    *,
    ...
    identifier = _first_non_empty(
        args.identifier,
        env_or(dotenv, "MIMISEE_SMOKE_IDENTIFIER", ""),
        env_or(dotenv, "MIMISEE_DEMO_IDENTIFIER", ""),
    ).strip()
    password = _first_non_empty(
        args.password,
        env_or(dotenv, "MIMISEE_SMOKE_PASSWORD", ""),
        env_or(dotenv, "MIMISEE_DEMO_PASSWORD", ""),
    ).strip()
    ...
    raise ValueError(
        "AUTH_MODE=jwt but no token provided. Set --token / MIMISEE_SMOKE_TOKEN, "
...
def _resolve_auth_headers(
    ...
    *,
    ...
    token = _first_non_empty(
        args.token,
        env_or(dotenv, "MIMISEE_SMOKE_TOKEN", ""),
        env_or(dotenv, "MIMISEE_DEMO_TOKEN", ""),
    ).strip()
...
def _upload_smoke_document(
    ...
    *,
    ...
    doc_text = (
        "MimiSee smoke test document (synthetic; no PII).\n\n"
