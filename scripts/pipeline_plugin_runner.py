...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Test a MimiSee pipeline plugin package locally.")
    sub = parser.add_subparsers(dest="command", required=True)

    test = sub.add_parser("test", help="Run a local pipeline plugin test and write the test report.")
    test.add_argument("plugin_dir", help="Plugin package directory containing mimisee-plugin.json/yaml.")
    test.add_argument(
    ...
    test.add_argument(
        "--no-write-report", action="store_true", help="Run test without writing .mimisee-plugin-test.json."
    )

    golden = sub.add_parser("golden-draft", help="Build a local Golden regression bundle from a plugin sample.")
    golden.add_argument("plugin_dir", help="Plugin package directory containing mimisee-plugin.json/yaml.")
    golden.add_argument(
        "--input", required=True, dest="input_path", help="JSON sample input: array or {documents:[...]}."
    )
    golden.add_argument(
