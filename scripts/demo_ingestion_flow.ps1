param(
  ...
  [string]$Identifier = $env:MIMISEE_DEMO_IDENTIFIER,
  [string]$Password = $env:MIMISEE_DEMO_PASSWORD,
  [string]$Token = $env:MIMISEE_DEMO_TOKEN,
  [string]$DatasetName = "",
  [string]$FilePath = "",
...
function Get-AccessToken {
  ...
  if (-not $Identifier -or -not $Password) {
    throw "Missing auth. Provide -Token or set -Identifier/-Password (or env: MIMISEE_DEMO_IDENTIFIER / MIMISEE_DEMO_PASSWORD)."
  }
