...
def _is_mimisee_http_result_node(title: str, node_type: str, outputs: Any) -> bool:
    if node_type != "code" or not isinstance(outputs, dict):
        return False
    result = outputs.get("result")
    if not isinstance(result, list):
        return False
    if "MimiSee结果转换" in title:
        return True
    for item in result:
        if not isinstance(item, dict):
            continue
        metadata = item.get("metadata") if isinstance(item.get("metadata"), dict) else {}
        if _text(metadata.get("retriever_from")) == "mimisee_http":
            return True
    return False
...
def _summarize_executions(executions: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    for execution in executions:
        ...
        if node_type == "knowledge-retrieval" or _is_mimisee_http_result_node(title, node_type, outputs):
...
def collect_trace_report(
    *,
    ...
    return {
        "schema": "mimisee.changzhou_gov_service_knowledge.dify_trace_report.v1",
