...
request reaches MimiSee retrieval. This script keeps that boundary diagnosable.
"""
...
_ANSWER_COVERAGE_PROMPT_MARKER = "MimiSee evidence coverage rules v1"
...
def _build_retrieval_payload_node(node: dict[str, Any], payload: dict[str, Any]) -> dict[str, Any] | None:
    ...
    title = _text(data.get("title")).replace("MimiSee HTTP检索", "安全构造 MimiSee 检索请求", 1)
    return {
        "data": {
            "code": code,
            "code_language": "python3",
            "desc": "Build a JSON-safe MimiSee retrieval payload. Keeps user text out of raw JSON templates.",
...
def patch_http_json_template_bodies(workflow: dict[str, Any]) -> tuple[dict[str, Any], list[dict[str, Any]]]:
    """Return a workflow copy with MimiSee retrieval JSON bodies built in Code nodes."""
    patched_workflow = copy.deepcopy(workflow)
...
def lint_workflow(workflow: dict[str, Any], *, cases: list[dict[str, Any]] | None = None) -> dict[str, Any]:
    ...
    report: dict[str, Any] = {
        "schema": "mimisee.changzhou_gov_service_knowledge.dify_workflow_lint.v1",
        "summary": summary,
