...
WEB_CONTAINER="${MIMISEE_WEB_CONTAINER:-mimisee-web-smoke-${GITHUB_RUN_ID:-local}-${GITHUB_RUN_ATTEMPT:-1}}"
...
if [[ -n "${MIMISEE_SMOKE_IDENTIFIER:-}" || -n "${MIMISEE_SMOKE_PASSWORD:-}" ]]; then
  if [[ -z "${MIMISEE_SMOKE_IDENTIFIER:-}" || -z "${MIMISEE_SMOKE_PASSWORD:-}" ]]; then
    echo "[jwt-browser-smoke] MIMISEE_SMOKE_IDENTIFIER and MIMISEE_SMOKE_PASSWORD must be set together" >&2
    exit 1
  fi
  jwt_identifier="$MIMISEE_SMOKE_IDENTIFIER"
  jwt_email=""
  jwt_password="$MIMISEE_SMOKE_PASSWORD"
  reuse_account=1
else
...
const password = `MimiSee-${crypto.randomBytes(24).toString('base64url')}`
...
if [[ -n "${GITHUB_ENV:-}" && "$reuse_account" == "0" ]]; then
  ...
  fi
  {
    ...
    printf 'MIMISEE_SMOKE_IDENTIFIER=%s\n' "$jwt_identifier"
    printf 'MIMISEE_SMOKE_PASSWORD=%s\n' "$jwt_password"
  } >>"$GITHUB_ENV"
fi
