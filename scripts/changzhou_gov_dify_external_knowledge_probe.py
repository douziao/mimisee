#!/usr/bin/env python3
"""Compare Dify external dataset hit-testing with direct MimiSee retrieval.

This is a read-only boundary diagnostic. It does not modify a Dify workflow or
MimiSee data; it only proves which side of the Dify external-knowledge boundary
...
RequestMimiSeeDirectFn = Callable[..., dict[str, Any]]
...
def _select_external_api(payload: dict[str, Any], external_api_id: str) -> dict[str, Any]:
    ...
    mimisee_apis = [item for item in apis if isinstance(item, dict) and "mimisee" in _text(item.get("name")).lower()]
    if len(mimisee_apis) == 1:
        return mimisee_apis[0]
    raise ValueError("Pass --external-api-id when Dify has zero or multiple MimiSee external APIs")
...
def _diagnosis(*, dify_count: int, direct_count: int, missing_dataset: bool, error: str) -> str:
    ...
    if dify_count == 0 and direct_count > 0:
        return "dify_runtime_empty_but_mimisee_direct_ok"
    if dify_count > 0:
...
def evaluate_probe_gate(report: dict[str, Any]) -> dict[str, Any]:
    ...
    if int(summary.get("mimisee_direct_nonempty") or 0) != cases:
        failed_conditions.append("mimisee_direct_nonempty")
    if int(summary.get("mimisee_direct_schema_valid") or 0) != cases:
        failed_conditions.append("mimisee_direct_schema_valid")
...
def build_boundary_verdict(report: dict[str, Any]) -> dict[str, Any]:
    ...
    local_mimisee_direct_ok = (
        cases > 0
        and int(summary.get("mimisee_direct_nonempty") or 0) == cases
        and int(summary.get("mimisee_direct_schema_valid") or 0) == cases
        and int(summary.get("probe_errors") or 0) == 0
    )
    dify_hit_testing_ok = cases > 0 and int(summary.get("dify_hit_nonempty") or 0) == cases
    if endpoint_config_ok and local_mimisee_direct_ok and dify_hit_testing_ok:
        verdict = "dify_external_boundary_ok"
    elif not endpoint_config_ok:
        verdict = "endpoint_config_invalid"
    elif local_mimisee_direct_ok and int(summary.get("dify_runtime_empty_but_mimisee_direct_ok") or 0) > 0:
        verdict = "dify_runtime_empty_but_mimisee_direct_ok"
    elif not local_mimisee_direct_ok:
        verdict = "mimisee_direct_unhealthy"
    else:
        verdict = "dify_hit_testing_unhealthy"
    return {
        "endpoint_config_ok": endpoint_config_ok,
        "local_mimisee_direct_ok": local_mimisee_direct_ok,
...
def collect_probe_report(
    *,
    ...
    console_token: str,
    request_json: RequestJsonFn = _request_json,
    request_mimisee_direct: RequestMimiSeeDirectFn = _request_mimisee_direct,
        ...
        for attempt in range(_REQUEST_ATTEMPTS):
            ...
            try:
                ...
                if endpoint and api_key and knowledge_id and query:
                    direct_payload = request_mimisee_direct(
                        endpoint=endpoint,
                        api_key=api_key,
                        knowledge_id=knowledge_id,
                        query=query,
        ...
        rows.append(
            {
                ...
                "mimisee_direct_records": direct_count,
                "mimisee_direct_schema_valid": not direct_schema_errors,
                "mimisee_direct_schema_errors": direct_schema_errors,
                "mimisee_direct_first_title": _first_record_title(direct_payload),
                "diagnosis": diagnosis,
    ...
    report = {
        "schema": "mimisee.changzhou_gov_service_knowledge.dify_external_knowledge_probe.v1",
        ...
        "summary": {
            ...
            "mimisee_direct_nonempty": sum(1 for row in rows if int(row.get("mimisee_direct_records") or 0) > 0),
            "mimisee_direct_schema_valid": sum(1 for row in rows if bool(row.get("mimisee_direct_schema_valid"))),
            "dify_runtime_empty_but_mimisee_direct_ok": sum(
                1 for row in rows if row.get("diagnosis") == "dify_runtime_empty_but_mimisee_direct_ok"
            ),
...
def _progress_to_stderr(event: dict[str, Any]) -> None:
    ...
    print(
        ...
        f"mimisee={event.get('mimisee_direct_records')} diagnosis={_text(event.get('diagnosis'))}",
        file=sys.stderr,
        flush=True,
    )
...
def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Probe Dify external knowledge hit-testing against direct MimiSee retrieval."
    )
