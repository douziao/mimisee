...
class LiveApi:
    ...
    def parse_preview(self, pdf_path: Path, backend: str) -> tuple[int, Any, float]:
        boundary = f"----MimiSeeParserMatrix{uuid.uuid4().hex}"
...
def write_fixture_pdf(path: Path, *, pages: int = 2) -> None:
    ...
    for page in range(1, pages + 1):
        page_texts.append(
            [
                f"MimiSee parser matrix page {page}",
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run a parser service matrix against a live MimiSee API.")
