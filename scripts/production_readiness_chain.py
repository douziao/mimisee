    ...
    for source in SOURCES:
        ...
        if not out.exists() or out.stat().st_size < 64:
            response = requests.get(
                source.url,
                timeout=60,
                headers={"User-Agent": "MimiSee-production-readiness/1.0"},
            )
...
def generate_office_files(corpus_dir: Path, evidence: Evidence) -> list[Path]:
    ...
    doc = Document()
    doc.add_heading("MimiSee Production Readiness Brief", level=1)
...
def write_report(evidence: Evidence) -> None:
    ...
    lines = [
        "# MimiSee Production Readiness Chain Evidence",
        "",
