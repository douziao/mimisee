...
It also exports a portable regression cases bundle (mimisee.regression_cases.v1) which can be
...
def build_cases_bundle(fixture: dict[str, Any]) -> dict[str, Any]:
    ...
    return {"schema": "mimisee.regression_cases.v1", "dataset_id": ds, "items": items}
...
def main() -> int:
    ...
    p.add_argument(
        "--out-cases",
        default="",
        help="Write regression cases bundle (mimisee.regression_cases.v1) to this path (optional)",
    )
    args = p.parse_args()
