#!/usr/bin/env python3
"""Benchmark a locally configured MimiSee retriever and LLM on the fixed 800-case pack.

The target MimiSee instance must already be configured with the embedding and reranker
...
def _call_case(
    *,
    ...
    retrieval = bench._call_mimisee_case(  # noqa: SLF001
        case=case,
        base_url=mimisee_base_url,
        token=mimisee_token,
        timeout=timeout,
    )
    item = dict(retrieval)
    ...
    try:
        ...
        item["ok"] = False
        item["error"] = bench._safe_error(exc, secrets=[llm_api_key, mimisee_token])  # noqa: SLF001
...
def _run_payload(
    *,
    ...
    return {
        "schema": "mimisee.local_3model_benchmark.run.v1",
...
def run_benchmark(
    *,
    ...
    items = list(reusable)
    source = {
        "pipeline": "MimiSee External Knowledge retrieval -> local OpenAI-compatible LLM",
        "mimisee_base_url": mimisee_base_url.rstrip("/"),
    ...
    with ThreadPoolExecutor(max_workers=max(1, concurrency)) as executor:
        futures = [
            executor.submit(
                _call_case,
                case=case,
                mimisee_base_url=mimisee_base_url,
                mimisee_token=mimisee_token,
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--mimisee-token", default=os.getenv("MIMISEE_TOKEN") or "")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not _text(args.mimisee_token):
        raise SystemExit("MIMISEE_TOKEN or --mimisee-token is required")
    ...
    run = run_benchmark(
        cases=cases,
        run_path=out_dir / "run_mimisee_local_3model.json",
        mimisee_base_url=str(args.mimisee_base_url),
        mimisee_token=str(args.mimisee_token),
    ...
    summary = {
        "schema": "mimisee.local_3model_benchmark.summary.v1",
        "generated_at": _utc_now_text(),
