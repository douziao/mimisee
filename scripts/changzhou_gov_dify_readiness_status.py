...
def _stages_with_status(report: dict[str, Any], status: str) -> list[str]:
    stages = ("knowledge_map", "mimisee_direct", "kg_compare", "console_auth", "external_probe", "full_gate")
    out: list[str] = []
    for stage in stages:
...
def format_markdown_evidence(
    ...
    *,
    ...
    mimisee_direct = _report_dict(report, "mimisee_direct")
    ...
    direct_summary = _report_dict(mimisee_direct, "summary")
    ...
    lines = [
        "# Changzhou Dify/MimiSee Readiness Evidence",
        "",
        ...
        [
            ...
            *_markdown_table(
                ...
                [
                    ...
                    [
                        "mimisee_direct",
                        _stage_status(report, "mimisee_direct"),
...
def _direct_base_line(report: dict[str, Any], external_probe: dict[str, Any]) -> str:
    mimisee_direct = _report_dict(report, "mimisee_direct")
    mimisee_source = _report_dict(mimisee_direct, "source")
    direct_base_url = _text(mimisee_source.get("base_url"))
    if not direct_base_url:
        return ""
    direct_base_host = _text(mimisee_source.get("base_host"))
    ...
    if external_endpoint_host and direct_base_host and direct_base_host != external_endpoint_host:
        return f"MimiSee direct base: {direct_base_url} (differs from external endpoint host {external_endpoint_host})"
    if external_endpoint_host and direct_base_host == external_endpoint_host:
        return f"MimiSee direct base: {direct_base_url} (matches external endpoint host)"
    return f"MimiSee direct base: {direct_base_url}"
