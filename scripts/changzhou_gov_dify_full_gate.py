#!/usr/bin/env python3
"""Run the full Changzhou Dify/MimiSee golden gate.

The gate composes existing diagnostics in the order operators need them:
case-input preflight, Dify generated-answer collection, direct MimiSee golden
...
def _finalize(stages: dict[str, dict[str, Any]]) -> dict[str, Any]:
    ...
    return {
        "schema": "mimisee.changzhou_gov_service_knowledge.dify_full_gate.v1",
        "summary": {
            "passed": not failed,
...
def compact_summary(
    ...
    *,
    ...
    return {
        "schema": "mimisee.changzhou_gov_service_knowledge.dify_full_gate.summary.v1",
...
def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run the full Changzhou Dify/MimiSee golden gate.")
    ...
    parser.add_argument("--mimisee-base-url", default=os.getenv("MIMISEE_API_BASE_URL") or "http://127.0.0.1:8000")
    parser.add_argument("--mimisee-token", default=os.getenv("DIFY_EXTERNAL_KNOWLEDGE_API_KEY") or "")
    ...
    parser.add_argument("--user", default="mimisee-full-gate")
    parser.add_argument(
        "--quality-profile",
...
def main(argv: list[str] | None = None) -> int:
    ...
    mimisee_token = load_mimisee_token(str(args.mimisee_token), env_file=str(REPO_ROOT / ".env"))
    if not dify_api_key:
    ...
    if not console_token:
        ...
        return 2
    if not mimisee_token:
        print("DIFY_EXTERNAL_KNOWLEDGE_API_KEY or --mimisee-token is required", file=sys.stderr)
        return 2
