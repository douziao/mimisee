#!/usr/bin/env python3
"""Run MimiSee-direct benchmark on the same case pack with KG off/on and compare.
...
- runs the local MimiSee direct retrieval benchmark twice with request-level KG overrides
...
def _mimisee_direct_report(
    *,
    ...
    run = bench.run_mimisee_direct(
        cases=cases,
        base_url=base_url.rsplit("/api/v1", 1)[0],
        token=bench.load_mimisee_token("", env_file=env_file),
        timeout=timeout,
...
def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run local MimiSee benchmark with KG off/on over the same case pack.")
...
def main(argv: list[str] | None = None) -> int:
    ...
    for mode, enabled in (("kg_off", False), ("kg_on", True)):
        run_path = out_dir / f"run_mimisee_direct_{mode}.json"
        _run, report = _mimisee_direct_report(
            cases=cases,
    ...
    comparison = {
        "schema": "mimisee.changzhou_gov.kg_on_off_benchmark.v1",
        "cases": len(cases),
