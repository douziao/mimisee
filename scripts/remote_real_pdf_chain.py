...
class LiveApi:
    ...
    def multipart(
        ...
        *,
        ...
        boundary = f"----MimiSeeRealPdf{uuid.uuid4().hex}"
        chunks: list[bytes] = []
...
def download(url: str, target: Path, timeout: int) -> dict[str, Any]:
    ...
    request = Request(url, headers={"User-Agent": "MimiSee real PDF chain"})
    started = time.perf_counter()
    with urlopen(request, timeout=timeout) as response:
        data = response.read()
...
def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=("Run a real-PDF parse -> chunk -> KG -> chat chain against a live MimiSee API.")
    )
