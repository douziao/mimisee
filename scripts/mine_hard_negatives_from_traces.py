...
Inputs:
- Regression cases bundle (mimisee.regression_cases.v1 or legacy shapes) provides:
...
Output:
- JSONL with schema mimisee.hard_negatives.v1
...
def _iter_feedback_event_rows(path: Path, *, max_records: int = 0) -> Iterator[dict[str, Any]]:
    ...
    - mimisee.training_export_row.v1 (feedback source rows)
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument(
        ...
        default="",
        help=(
            "Optional JSONL with feedback/training rows (e.g. mimisee.training_export_row.v1) "
