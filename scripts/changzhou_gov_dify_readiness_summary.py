...
SCHEMA = "mimisee.changzhou_gov_service_knowledge.dify_readiness_summary.v1"
_RETRIEVAL_AUDIT_SAFE_METRIC_KEYS = {
    ...
    "hit_at_20",
    "kg_noise_rate",
    "mimisee_direct_nonempty",
    "mimisee_direct_schema_valid",
    "probe_errors",
...
def _mimisee_direct_section(report: dict[str, Any]) -> dict[str, Any]:
    gate = report.get("gate") if isinstance(report.get("gate"), dict) else {}
    summary = report.get("summary") if isinstance(report.get("summary"), dict) else {}
    source = report.get("source") if isinstance(report.get("source"), dict) else {}
    failed_conditions = _quality_gate_failed_conditions(report)
    if not gate and not summary:
        failed_conditions.append("mimisee_direct_report_missing")
    out = {
...
def _adapter_category_failed(gates: list[dict[str, Any]]) -> bool:
    direct_nonempty = _first_metric(gates, "mimisee_direct_nonempty")
    external_nonempty = _first_metric(gates, "dify_hit_nonempty")
    schema_valid = _first_metric(gates, "mimisee_direct_schema_valid")
...
def _build_retrieval_audit(
    *,
    ...
    mimisee_direct_raw: dict[str, Any] | None,
    kg_compare_raw: dict[str, Any] | None,
    sections_by_stage: dict[str, dict[str, Any]],
) -> dict[str, Any]:
    gates: list[dict[str, Any]] = []
    for stage in ("knowledge_map", "mimisee_direct", "external_probe", "full_gate"):
    ...
    raw_sections = [
        section for section in (knowledge_map_raw or {}, mimisee_direct_raw or {}) if isinstance(section, dict)
    ]
    audit = {
...
_STATIC_NEXT_ACTIONS = {
    ...
    "mimisee_direct": (
        "Run make changzhou-dify-mimisee-direct-gate and inspect /tmp/changzhou_gov_dify_mimisee_direct_gate.json."
    ),
...
def _knowledge_map_next_action(reason: str) -> str:
    ...
    if reason.startswith("plugin_retrieval_policy_missing:"):
        return (
            "Add a mimisee.retrieval_policy.v1 retrieval_policy to the referenced plugin manifest, "
            "then run make changzhou-dify-knowledge-map-check."
        )
...
def _external_probe_next_action(reason: str) -> str:
    if reason in {"endpoint_missing", "endpoint_host_is_loopback"}:
        return "Set Dify external knowledge endpoint to a MimiSee URL reachable from the Dify server, not localhost."
...
def _build_stage_sections(
    *,
    ...
    mimisee_direct: dict[str, Any] | None,
    ...
    return (
        ...
        _section_or_none(_mimisee_direct_section, mimisee_direct),
...
def _staged_sections(
    *,
    ...
    mimisee_section: dict[str, Any] | None,
    ...
    for stage, section in (
        ...
        ("mimisee_direct", mimisee_section),
...
def _artifact_report_sources(
    *,
    ...
    return {
        ...
        "mimisee_direct": mimisee_direct or {},
...
def build_readiness_summary(
    *,
    ...
    mimisee_direct: dict[str, Any] | None = None,
    ...
    staged_sections = _staged_sections(
        ...
        mimisee_section=mimisee_section,
    ...
    report["retrieval_audit"] = _build_retrieval_audit(
        ...
        mimisee_direct_raw=mimisee_direct,
    ...
    if mimisee_section is not None:
        report["mimisee_direct"] = sections_by_stage["mimisee_direct"]
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--mimisee-direct", default="")
...
def main(argv: list[str] | None = None) -> int:
    ...
    artifact_paths = {
        ...
        "mimisee_direct": str(args.mimisee_direct),
    ...
    report = build_readiness_summary(
        ...
        mimisee_direct=artifact_reports.get("mimisee_direct") if _text(args.mimisee_direct) else None,
