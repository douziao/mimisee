...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--base-url",
        default=os.getenv("MIMISEE_API_BASE_URL") or os.getenv("BACKEND_BASE_URL") or "http://127.0.0.1:8000",
        help="Backend base URL; may be root or /api/v1 (default: env MIMISEE_API_BASE_URL/BACKEND_BASE_URL or http://127.0.0.1:8000)",
    )
    ...
    parser.add_argument("--tenant-id", default=os.getenv("MIMISEE_TENANT_ID") or DEFAULT_TENANT_ID)
    parser.add_argument("--account-id", default=os.getenv("MIMISEE_ACCOUNT_ID") or "demo")
    parser.add_argument("--user-id", default=os.getenv("MIMISEE_USER_ID") or "demo")
    parser.add_argument("--bearer", default=os.getenv("MIMISEE_API_TOKEN") or os.getenv("AUTH_TOKEN") or "")
    parser.add_argument("--timeout", type=float, default=float(os.getenv("MIMISEE_API_TIMEOUT_SEC") or "60"))
    parser.add_argument("--poll-timeout", type=float, default=float(os.getenv("MIMISEE_POLL_TIMEOUT_SEC") or "600"))
    parser.add_argument("--poll-interval", type=float, default=float(os.getenv("MIMISEE_POLL_INTERVAL_SEC") or "2"))
