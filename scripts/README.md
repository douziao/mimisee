...
- `plugin_release_gate.py`: generic local pipeline-plugin release gate
  ...
  - Runs the local stage test, verifies the written `.mimisee-plugin-test.json` still matches the current plugin package hash, builds the generic chunk readiness report, and checks declared Golden rules produce a draft.
...
- `changzhou_gov_delivery_pack.py`: combined handoff index for the Changzhou plugin evidence and Dify/MimiSee readiness evidence
  ...
  - Use `make changzhou-gov-delivery-pack-refresh` for delivery handoff: it runs `changzhou-dify-readiness-gate-quiet` first, sending raw Dify/MimiSee gate output to `/tmp/changzhou_gov_dify_readiness_gate.log`.
...
- `changzhou_gov_dify_external_knowledge_probe.py`: compare Dify external hit-testing with direct MimiSee retrieval for the same Changzhou golden cases
  - MimiSee-only preflight: `make changzhou-dify-mimisee-direct-gate` runs the same golden retrieval cases directly against MimiSee using `DIFY_EXTERNAL_KNOWLEDGE_API_KEY(S)` from `.env`, without Dify console auth. The default command enables the `changzhou-retrieval` quality profile, so it gates hit rates, evidence quality (`answer_grounding_rate`, key-point recall, effective context rate, and noise rate), and expected-metadata coverage/match rates for cases that declare `expected.metadata`.
  - Existing-report gate: `python scripts/changzhou_gov_golden_eval.py --report /tmp/report.json --quality-profile changzhou-retrieval` rechecks a saved report without calling MimiSee or requiring a token.
  - KG-on/KG-off comparison gate: `make changzhou-dify-kg-on-off-gate` first writes KG-off and KG-on MimiSee-only reports with `--kg-mode off/on`, then compares them. The saved-report-only form remains available as `CHANGZHOU_DIFY_KG_BASELINE_REPORT=/tmp/kg-off.json CHANGZHOU_DIFY_KG_CANDIDATE_REPORT=/tmp/kg-on.json CHANGZHOU_DIFY_KG_COMPARE_OUT=/tmp/changzhou_gov_dify_kg_compare.json make changzhou-dify-kg-compare-gate`.
  - `--kg-mode` is a MimiSee direct-gate extension: `default` inherits service settings; `off/on` sends request-level KG overrides to the Dify adapter. Standard Dify requests can omit these fields.
  ...
  - The command exits non-zero unless the Dify endpoint host is non-loopback, all cases have non-empty Dify hit-testing results, direct MimiSee retrieval is non-empty, and direct records match Dify's external knowledge schema.
...
- `changzhou_gov_dify_full_gate.py`: run the Changzhou government-service Dify/MimiSee golden gate (preflight, generated answers, direct eval, workflow trace)
  ...
  - Quick diagnosis: `make changzhou-dify-readiness-status` prints the latest summary's pass/fail state, freshness, boundary verdict, direct MimiSee base URL match state, non-blocking warning counters, affected case ids, advisory warning diagnoses/details, root cause, next action, skipped stages, and artifact paths.
  - If readiness status says the direct base differs from the external endpoint host, rerun with `CHANGZHOU_DIFY_MIMISEE_BASE_URL=<MimiSee base used by Dify>` so the direct gate and Dify boundary probe compare the same service instance.
...
- `evaluate_mixed_rag_quality.py`: compare complex mixed RAG questions across MimiSee/Dify runs using objective evidence and subquestion coverage, not an LLM judge.
  - Example: `MIXED_RAG_RUNS='mimisee=/tmp/mimisee_run.json dify=/tmp/dify_run.json' make mixed-rag-quality`
  - Override the case set with `MIXED_RAG_CASES=/tmp/mixed_cases.json`.
  - Case files use `{"schema":"mimisee.mixed_rag_eval_cases.v1","cases":[...]}`. Each case declares `question`, `subquestions[]`, and `evidence_clauses[]`; each clause/subquestion uses `required_terms` and optional `metadata`.
  ...
  - Run files use `{"schema":"mimisee.mixed_rag_eval_run.v1","system":"mimisee","items":[...]}`. Each item supplies `case_id`, optional `answer`, `records[]`, and optional `latency_ms`.
...
- `remote_chunking_matrix.py`: live chunking matrix against a running API; set `MIMISEE_REMOTE_LONG_PDF_FIXTURE=/path/to/long.pdf` to reuse a local long PDF, or set `MIMISEE_REMOTE_FIXTURE_DOWNLOADS=1` to download the RFC sample.
...
- `mine_hard_negatives_from_traces.py`: mine PII-safe hard negatives from `event=rag_trace` metrics logs (outputs `mimisee.hard_negatives.v1` JSONL)
...
- `run_sample_retrieval_benchmark.py`: run a deterministic local sample benchmark from `data/sample/retrieval_fixture_v1.json`
  - Outputs: `runs/sample_bench.json` (schema: `mimisee.sample_retrieval_benchmark.v1`)
...
- `run_queryset_health_diagnostics.py`: build query-set health snapshot from benchmark report and maintain bounded trend history
  - Output snapshot schema: `mimisee.queryset_health_snapshot.v1`
...
- `diff_queryset_health_snapshots.py`: diff baseline/current query-set health snapshots for PR/release review
  - Output schema: `mimisee.queryset_health_diff.v1`
