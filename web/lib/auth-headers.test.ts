...
describe('getAuthHeaders', () => {
  ...
  it('prefers bearer auth and includes tenant context', () => {
    localStorage.setItem('mimisee_tenant_id', 'tenant-1')
  ...
  it('does not emit shared auth metadata after another tab broadcasts logout', () => {
    ...
    expect(localStorage.getItem('mimisee_user_id')).toBe('user-1')
    expect(localStorage.getItem('mimisee_tenant_id')).toBe('tenant-1')
  ...
  it('does not emit shared auth metadata after another tab switches users', () => {
    ...
    expect(getAccessToken()).toBeNull()
    expect(localStorage.getItem('mimisee_user_id')).toBe('user-b')
    expect(localStorage.getItem('mimisee_tenant_id')).toBe('tenant-a')
    expect(getAuthHeaders()).toEqual({})
  })
})
