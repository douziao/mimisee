...
describe('auth storage scope', () => {
  ...
  it('preserves an explicitly selected tenant for the first login only', () => {
    localStorage.setItem('mimisee_tenant_id', 'tenant-a')
  ...
  it('clears a stale tenant exactly once when switching to another user and on logout', () => {
    ...
    localStorage.setItem('mimisee_document_view_v1', '{"private":true}')
  ...
  it('stores the access token in sessionStorage instead of localStorage', () => {
    ...
    expect(sessionStorage.getItem('mimisee_access_token')).toBe('token')
    expect(localStorage.getItem('mimisee_access_token')).toBeNull()
    expect(sessionStorage.getItem('mimisee_token_expires_at')).toBeTruthy()
    expect(localStorage.getItem('mimisee_token_expires_at')).toBeNull()
  })

  it('migrates a legacy localStorage access token into sessionStorage on read', () => {
    localStorage.setItem('mimisee_access_token', 'legacy-token')
    localStorage.setItem('mimisee_token_expires_at', '123')
    localStorage.setItem('mimisee_user_profile', JSON.stringify({ id: 'legacy-user' }))
    localStorage.setItem('mimisee_user_id', 'legacy-user')

    expect(getAccessToken()).toBe('legacy-token')
    expect(sessionStorage.getItem('mimisee_access_token')).toBe('legacy-token')
    expect(sessionStorage.getItem('mimisee_token_expires_at')).toBe('123')
  ...
  it('keeps the legacy token when sessionStorage is unavailable during migration', () => {
    ...
    try {
      expect(getAccessToken()).toBe('legacy-token')
      expect(getStoredUser()).toEqual({ id: 'legacy-user' })
      expect(localStorage.getItem('mimisee_access_token')).toBe('legacy-token')
      expect(localStorage.getItem('mimisee_token_expires_at')).toBe('123')
    } finally {
  ...
  it('clears legacy and session token storage on logout', () => {
    ...
    expect(sessionStorage.getItem('mimisee_access_token')).toBeNull()
    expect(sessionStorage.getItem('mimisee_token_expires_at')).toBeNull()
    expect(localStorage.getItem('mimisee_access_token')).toBeNull()
    expect(localStorage.getItem('mimisee_token_expires_at')).toBeNull()
  })
  ...
  it('does not expose a persisted user profile or authenticated scope without a token', () => {
    localStorage.setItem('mimisee_user_profile', JSON.stringify({ id: 'user-a', email: 'user@example.com' }))
