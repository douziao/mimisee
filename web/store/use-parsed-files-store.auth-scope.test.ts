...
vi.mock('@/lib/auth-storage', () => ({
  AUTH_SCOPE_CHANGED_EVENT: 'mimisee:auth-scope-changed',
...
describe('useParsedFiles auth scope persistence', () => {
  ...
  it('persists parsed file metadata under the active auth scope key', () => {
    ...
    expect(localStorage.getItem('mimisee_parsed_files:default:anonymous')).toContain('same-scope.md')
    expect(localStorage.getItem('mimisee_parsed_files')).toBeNull()
  })

  it('resets and rehydrates from the new auth scope when the scope changes', async () => {
    localStorage.setItem(
      'mimisee_parsed_files:tenant-b:user-b',
    ...
    window.dispatchEvent(new Event('mimisee:auth-scope-changed'))
  ...
  it('drops legacy unscoped state instead of exposing it in anonymous scope', async () => {
    localStorage.setItem('mimisee_parsed_files', makePersistedState('legacy-anon.md'))
    ...
    expect(localStorage.getItem('mimisee_parsed_files')).toBeNull()
    expect(
      localStorage.getItem('mimisee_parsed_files:default:anonymous')?.includes('legacy-anon.md') ?? false
    ).toBe(false)
  })

  it('drops legacy unscoped state instead of exposing it in an authenticated scope', async () => {
    localStorage.setItem('mimisee_parsed_files', makePersistedState('legacy-private.md'))
    ...
    expect(localStorage.getItem('mimisee_parsed_files')).toBeNull()
    expect(
      localStorage.getItem('mimisee_parsed_files:tenant-a:user-a')?.includes('legacy-private.md') ?? false
    ).toBe(false)
  })

  it('removes the legacy unscoped key when scoped storage is cleared', () => {
    localStorage.setItem('mimisee_parsed_files', makePersistedState('legacy-private.md'))
    localStorage.setItem('mimisee_parsed_files:tenant-a:user-a', makePersistedState('tenant-a-file.md'))
    authMock.scope = 'tenant-a:user-a'

    useParsedFiles.persist.clearStorage()

    expect(localStorage.getItem('mimisee_parsed_files')).toBeNull()
    expect(localStorage.getItem('mimisee_parsed_files:tenant-a:user-a')).toBeNull()
  })
  ...
  it('keeps scoped persistence blocked until the latest rehydrate finishes', async () => {
    ...
    try {
      ...
      expect(localStorage.getItem('mimisee_parsed_files:tenant-c:user-c')).toBeNull()
      ...
      expect(localStorage.getItem('mimisee_parsed_files:tenant-c:user-c')).toContain('latest-scope.md')
    } finally {
