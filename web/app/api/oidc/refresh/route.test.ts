...
function makeRequest() {
  return new NextRequest('https://app.example.com/api/oidc/refresh', {
    ...
    headers: {
      ...
      cookie: 'mimisee_oidc_refresh_token=refresh-token; mimisee_oidc_provider_id=default',
    },
  })
}
...
describe('OIDC refresh route', () => {
  ...
  it('clears refresh cookies only for terminal invalid_grant responses', async () => {
    ...
    expect(response.headers.get('set-cookie')).toContain('mimisee_oidc_refresh_token=')
    expect(response.headers.get('set-cookie')).toContain('Max-Age=0')
    expect(response.headers.get('set-cookie')).toContain('mimisee_oidc_provider_id=')
  })
