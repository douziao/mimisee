...
interface ChunkPreviewReviewReport {
  schema: 'mimisee.chunk_review.v1'
  generated_at: string
  file: {
    filename: string
    file_type: string
...
export function applyChunkOverridesToPreview(
  ...
  const chunks = (preview.chunks || []).reduce<ChunkPreviewItem[]>((acc, chunk, fallbackIndex) => {
    ...
    const exportMetadata = isDisabled && includeDisabled ? { ...metadata, __mimisee_skip: true } : metadata
    acc.push({
      ...chunk,
      content,
...
export function chunkPreviewToReviewReport(
  ...
  const chunks: ChunkPreviewReviewReportChunk[] = (merged.chunks || []).map((c) => {
    ...
    const override = overrides?.[idx]
    const isDisabled = Boolean(override?.disabled) || Boolean(meta.__mimisee_skip)
...
export function chunkPreviewToReviewMarkdown(
  ...
  lines.push(`# ${filename} — Chunk Review`, '', `- schema: ${String(report.schema || 'mimisee.chunk_review.v1')}`)
