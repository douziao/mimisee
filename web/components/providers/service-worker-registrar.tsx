...
export async function clearLocalMimiSeeCaches() {
  if (typeof caches === 'undefined') return

  const keys = await caches.keys()
  await Promise.all(keys.filter((key) => key.startsWith('mimisee-')).map((key) => caches.delete(key)))
}

export async function clearLocalMimiSeeServiceWorkerState() {
  if (typeof navigator === 'undefined' || !('serviceWorker' in navigator)) return

  const registrations = await navigator.serviceWorker.getRegistrations()
  await Promise.all(registrations.map((registration) => registration.unregister()))

  await clearLocalMimiSeeCaches()
}
...
function scheduleLocalMimiSeeServiceWorkerCleanup() {
  const runCleanup = () => {
    clearLocalMimiSeeServiceWorkerState().catch(reportServiceWorkerCleanupFailure)
  }
  ...
  useEffect(() => {
    ...
    if (!enabled) {
      if (shouldClearLocalServiceWorker(env)) {
        return scheduleLocalMimiSeeServiceWorkerCleanup()
      }
      return
    }
