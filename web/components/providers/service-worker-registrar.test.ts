...
describe('service worker policy', () => {
  ...
  it('deletes only MimiSee caches', async () => {
    const deleteCache = vi.fn().mockResolvedValue(true)
    vi.stubGlobal('caches', {
      keys: vi.fn().mockResolvedValue(['mimisee-v1', 'other-app']),
      delete: deleteCache,
    })

    await clearLocalMimiSeeCaches()
    expect(deleteCache).toHaveBeenCalledOnce()
    expect(deleteCache).toHaveBeenCalledWith('mimisee-v1')
  })
