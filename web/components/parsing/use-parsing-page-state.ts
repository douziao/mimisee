...
export function useParsingPageState({ files, setFiles }: Readonly<UseParsingPageStateOptions>) {
  ...
  useEffect(() => {
    ...
    const stored = globalThis.window.localStorage.getItem('mimisee_parsing_image_caption_enabled')
    if (stored === 'true') setImageCaptionEnabled(true)
    const imageOcrStored = globalThis.window.localStorage.getItem('mimisee_parsing_image_ocr_enabled')
    if (imageOcrStored === 'true') setImageOcrEnabled(true)
    const vlmCorrectionStored = globalThis.window.localStorage.getItem('mimisee_parsing_vlm_correction_enabled')
  ...
  useEffect(() => {
    ...
    globalThis.window.localStorage.setItem(
      'mimisee_parsing_image_caption_enabled',
  ...
  useEffect(() => {
    if (globalThis.window === undefined) return
    globalThis.window.localStorage.setItem(
      'mimisee_parsing_image_ocr_enabled',
  ...
  useEffect(() => {
    if (globalThis.window === undefined) return
    globalThis.window.localStorage.setItem(
      'mimisee_parsing_vlm_correction_enabled',
